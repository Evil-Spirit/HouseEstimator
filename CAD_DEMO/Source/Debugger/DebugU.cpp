...
TMDelTList<TLuaVariable> *AllLVars = new TMDelTList<TLuaVariable>;
TMDelTList<TLuaVariable> *AllGVars = new TMDelTList<TLuaVariable>;
TMDelTList<TLuaVariable> *AllStack = new TMDelTList<TLuaVariable>;

TLuaRegObject *AllTree = new TLuaRegObject;
TLuaModule* LM = NULL;
...
void  CloseDebug()
{
    ...
    if(DebugForm->LocalVars)
    DebugForm->LocalVars->Close();
    if(DebugForm->GlobalVars)
    DebugForm->GlobalVars->Close();
    if(DebugForm->StackWnd)
    DebugForm->StackWnd->Close();
    if(DebugForm->BreakPointsWnd)
    DebugForm->BreakPointsWnd->Close();
    if(DebugForm->WatchWnd)
    DebugForm->WatchWnd->Close();

    for (int i=0;i<DebugForm->MDIChildCount;i++)
        DebugForm->MDIChildren[i]->Close();
...
void  TDebugForm::ViewStackExecute(TObject *Sender)
{
    if (!StackViewItem->Checked)
    {
        ...
        else
        {
            ...
            StackWnd->Caption="Stack contents";
            StackWnd->Show();
            StackWnd->ViewStack(AllStack);
        }
    }
...
void  TDebugForm::ViewLocalVarsExecute(TObject *Sender)
{
    if (!LocalVariableItem->Checked)
    {
        ...
        {
            this->LocalVars = new TGlobal(this);
            this->LocalVars->Caption="Local variables";
            LocalVars->ViewVars(AllLVars);
        }
    }
}
...
void  TDebugForm::ViewGlobalVarsExecute(TObject *Sender)
{
    if (!GlobalVariableItem->Checked)
    {
        ...
        else
        {
            ...
            GlobalVars->Caption="Global variables";
...
void  StateRead(int Line)
{
    ...
    if((DebugForm->WatchWnd)&&(DebugForm->WatchWnd->Visible))
        DebugForm->WatchWnd->Evaluate(DebugForm);
}
...
{
    AnsiString Text = InputBox("Quick evaluate", "Inpute code", "");
    if (Text!="") Inspect(Text);
}

void  TDebugForm::Button1Click(TObject *Sender)
{
    ApplHnd=GetWindowThreadProcessId(FindWindow("TConfig","3D House Estimator"),NULL);
    if (!ApplHnd) InfoMsg("Window not found!");
    else InfoMsg("Window "+IntToHex((int)ApplHnd,7));
    PostThreadMessage(ApplHnd,WM_USER,MSG_GETMODE,0);
    DebugMode=DBGMODE_WORK;
}
//---------------------------------------------------------------------------
