...
AnsiString TAdditionalInfinityParLine::OnHint()
{
    if (ExecutedId == 0 && PerLine)
    {
        ...
        return " Óãîë: "+FloatToStrF(Angle*180/M_PI, ffGeneral, 4, UIP.Sender->Grid.Precision)+" grad.";
    }
    if (ExecutedId == 3)
    {
        TIntVec V1 = Point;
...
//        int ICC = IsLinesCrossedExactly(Point1, Point2, V1, V2, V);
        ...
        return " Óãîë: "+FloatToStrF(Angle*180/M_PI, ffGeneral, 4, UIP.Sender->Grid.Precision)+"      Ñìåùåíèå: "+FloatToStrF(d, ffGeneral, 4, UIP.Sender->Grid.Precision);
    }
    return "";
}
