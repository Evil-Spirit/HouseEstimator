...
void ExcelExport(TMetaNode* MN,bool All,const AnsiString& FileName)
{
    ...
    _TRY_
            ...
            int rowshift = 1;
            StartProcess("Ýêñïîðò äàííûõ â Excel...",Fields.Count);
...
//                int MinSymbolCount = MaxSymbolCount;
            ...
            XL.OlePropertySet("DisplayAlerts",false); //îòêëþ÷èòü äèàãíîñòèêó ïðè çàêðûòèè ñåðâåðà
    ...
    _ENDTRY_("<ExcelExport>: Ýêñïîðò íå áûë çàâåðøåí",);
...
void ExcelImport(const AnsiString& FileName)
{
    ...
    _TRY_
        ...
        if (GUIDField == -1 || NameField==-1 )
            throw EMyException("Íå íàéäåíî ïîëå \"GUID\" èëè \"Name\". Èìïîðò ïðåðâàí.");
        StartProcess("Àíàëèç ðàáî÷åé êíèãè...",100);
        ...
        StartProcess("Ïðèåì äàííûõ ...",StrCount-1);
...
AnsiString DecodeHM(const TDateTime& DT)
{
    ...
    h = h + days*24;
    return (IntToStr(h)+AnsiString("÷.")+IntToStr(m))+AnsiString("ì.");
}
...
int OneAct(Variant& Sheet,int RowCount,TMTList<TQuerryNode>& QNS)
{
    ...
    _TRY_
        ...
        //--------------------------------------------------------------------------
        v2 = v1.OlePropertyGet("Cells").OlePropertyGet("Item",Current,ColCount);
        Str = Format("Premium %.2f ð.", ARRAYOFCONST( (TotalMoney*0.2) ) );
        v2.OlePropertySet("Value",Str.c_str());
        CellStyle(v2,HAlignRight,false,10,false);
        //--------------------------------------------------------------------------
        v2 = v1.OlePropertyGet("Cells").OlePropertyGet("Item",Current+1,1);
        ...
        Current = Current+3;
        v2 = v1.OlePropertyGet("Cells").OlePropertyGet("Item",Current,1);
        v2.OlePropertySet("Value","äëÿ àòòåñòàöèè áðèãàäû ïî íîðìàì âûïîëíåíèÿ ðàáîò ïî ñèñòåìå");
...
void Graphic( Variant& XL,const Variant& Sheet,TMTList<TQuerryNode>& QNL)
{
    ...
    _TRY_
        ...
        v1.OlePropertyGet("Range",v2,v3).OlePropertyGet("Columns").OleFunction("Group");
            ...
            v2.OlePropertySet("Value","Òðóäîâû Ðåñóðñû");
            CellStyle(v2,HAlignCenter,false,8,true);
            v2.OlePropertySet("WrapText",true);
            v2 = v1.OlePropertyGet("Cells").OlePropertyGet("Item",curcount,3);
            v2.OlePropertySet("Value","Ìåõàíèçìû");
            CellStyle(v2,HAlignCenter,false,8,true);
            v2.OlePropertySet("WrapText",true);
        ...
        StartProcess("Ñîçäàíèå ëèíåéíîãî ãðàôèêà...",QNL.Count*10);
    ...
    _ENDTRY_("<Graphic>: Îøèáêà ïðè ñîçäàíèè ãðàôèêà",);
    v1 = Unassigned;
    v2 = Unassigned;
    v3 = Unassigned;
}
...
/*    TQuickList L;
    ...
    try
    {
        try
        {
            ...
            v0=v1.OlePropertyGet("Worksheets") ;
            v1=v0.OlePropertyGet("Item",1);
            v1.OlePropertySet("Name","Àêòû ñîîòâåòñòâèÿ");

            int curcount = 0;
            StartProcess("Ñîçäàíèå àêòîâ ñîîòâåòñòâèÿ...",10*QNL.Count);
            for (int i=0;i<QNL.Count;i++)
            {
                curcount++;
                v2 = v1.OlePropertyGet("Cells").OlePropertyGet("Item",curcount,1);
                ...
                v2.OlePropertySet("Value",(AnsiString("ñòðàíèöà ")+IntToStr(i+1)+AnsiString(" èç ")+IntToStr(QNL.Count) ).c_str());
