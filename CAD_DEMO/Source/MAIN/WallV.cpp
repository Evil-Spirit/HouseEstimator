    ...
    :TArchitectElement(_MyTree,_Meta)
{
    ...
    {
		Application->MessageBox("Îæèäàëîñü MetaCube, ïîëó÷åíî Unknown. Ïðèëîæåíèå ÑÒÎÏ.","Error");
...
void TCube::Changed(void* Sender)
{
    ...
    if (Sender == &AbsSize)
    {
		...
		if (VHP.Vertex->Count==4) //return this (çàâèñèìîñòü ðàçìåðà îò ïîëèãîíà ì.á. ïî z áðàòü ñàìûé ìàëåíüêèé)
        {
...
{
	//----------------âàæíî äëÿ ðàáîòû àëãîðèòìà ñèíõðîíèçàöèè
...
//        FragSet[0].Polygons[0].Vertex->Add() //return this çàäàíèå ïî óìîë÷àíèþ
    }
    //----------------âàæíî äëÿ ðàáîòû àëãîðèòìà ñèíõðîíèçàöèè
    ...
    for (int i=0;i<SDL.Count;i++)
	{
		...
		//íå ìîæåì ñäåëàòü ïðîñòî àññèãí
        //ïîòîìó ÷òî ïåðâûé ïîëèãîí èñïîëüçóåò ïîëèãîí "is read only"
        //à íå êîïèðóåò ñïèñîê âåêòîðîâ ïðîåìà
        //à âî âðåìÿ ðàáîòû ñ îêíàìè ýòîò ñïèñîê ìåíÿåòñÿ
        //ïðîèñõîäèò îáðàùåíèå íå â òå ó÷àñòêè ïàìÿòè
    ...
    //òåïåðü ïîëó÷åíèå ïåðâûõ ïîëèãîíîâ
    for (int i=0;i<FragSet.Count;i++)
    {
