...
//---------------------------------------------------------------------------
bool TConfig1::Process_Save_Project()//âîçâðàùàåò èñòèíà åñëè îòâåò íå cancel
{
//-------------
    _TRY_
//-------------
    if (!World)
        return false;
    switch(Application->MessageBox( (AnsiString("Ñîõðàíèòü èçìåíåíèÿ â ïðîåêòå: ")+World->Name+AnsiString("?")).c_str(),"Confirmation",MB_YESNOCANCEL))
    {
        case 0:
        case IDCANCEL:
            return false;
...
//---------------------
    _ENDTRY_("<TConfig::Process_Save_Project>: Îøèáêà ïðè çàïèñè ôàéëà",return (Application->MessageBox("Îøèáêà ïðè çàïèñè ôàéëà, çàêðûòü âñå ðàâíî?","Confirmation",MB_YESNO) == IDYES););
//---------------------
...
/*              else if (ActiveMDIChild  && IS(ActiveMDIChild,__classid(TMDI2D)))
    ...
    _ENDTRY_("<TConfig::FileSaveAs1Execute>: Îøèáêà ïðè çàïèñè ôàéëà",);
}
//---------------------------------------------------------------------------
...
//    ShowClasses();
    ...
    _ENDTRY_("<TConfig::FileOpen1Execute>: Îøèáêà ïðè îòêðûòèè ôàéëà",);
//    ShowClasses();    
}
...
void  TConfig1::mWorkCountClick(TObject *Sender)
{
    ...
    if (!World->TechCheck(Str))
    {
        if (!Str.IsEmpty())
            ShowMessage(Str);
        ShowMessage("Ïðîâåðêà íà ñîîòâåòñòâèå òåõíîëîãèè íåçàâåðøåíà.");
        return;
    }
...
void  TConfig1::AppActionUpdate(Classes::TBasicAction* Action, bool &Handled)
{
    ...
    if (World)
        if (ActiveMDIChild && IS(ActiveMDIChild,__classid(TMDI3DUser)))
        {
            ActiveMDIChild->Caption = World->AskedToSave ? World->Name : World->Name + AnsiString(" (Èçìåíåí)");
        }
      ...
      TObject *Sender)
{
    ...
    if (startsel == endsel)
    {
        ShowMessage(AnsiString("Íå âûäåëåí òåêñò äëÿ çàìåíû."));
        return;
    }
...
{
    UndoStack.OpenSession("Äîáàâèòü ýòàæ ñâåðõó");
    World->AddFloor();
    UndoStack.CloseSession();
}
//---------------------------------------------------------------------------
...
void  TConfig1::N5Click(TObject *Sender)
{
    if (World->Floors.Count==1)
        return;
    UndoStack.OpenSession("Óäàëèòü àêòóàëüíûé ýòàæ");
