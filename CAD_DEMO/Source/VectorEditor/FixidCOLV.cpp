...
AnsiString TCreateObjectLines::OnHint()
{
    if (ExecutedId == 2)
    {
        ...
        return " ("+FloatToStrF(Point1.x, ffGeneral, 4, UIP.Sender->Grid.Precision)+"; "+FloatToStrF(fabs(Point1.y), ffGeneral, 4, UIP.Sender->Grid.Precision)+")  ("+FloatToStrF(Point2.x, ffGeneral, 4, UIP.Sender->Grid.Precision)+"; "+FloatToStrF(fabs(Point2.y), ffGeneral, 4, UIP.Sender->Grid.Precision)+")      Óãîë: "+FloatToStrF(Angle*180/M_PI, ffGeneral, 4, UIP.Sender->Grid.Precision);
    }
    return "";
}
