...
bool RUN_LUA(const AnsiString& Code)
{
    _TRY_
        ...
        if (__ERROR!=0)
            ShowError(NULL);
        return true;
    _ENDTRY_(AnsiString("<RUN_LUA>: Îøèáêà ïðè âûïîëíåíèè LUA-Ìîäóëÿ "),ShowError(NULL);return false;);
}
...
COMMONAL_API bool RUN_TEXT(TLuaModule *LM)
{
    ...
    _ENDTRY_(AnsiString("<RUN_BUFFER>: Îøèáêà ïðè âûïîëíåíèè LUA-Ìîäóëÿ ")+LM->Name,ShowError(LM);return false;);
}
...
/*    AnsiString NewAName;
    ...
    if (ReadOne(&NewAName,mtString,_MS) != -1)
          throw EMyException("Íåèçâåñòíàÿ îøèáêà");
...
bool TLuaModule::Run()
{
    if (HasForm)
    {
        ...
        if (!_Owner)
            _Owner = Config;
        if (!_Owner)
            throw EMyException("Ñîçäàíèå îáúåêòà áåç âëàäåëüöà");  */
