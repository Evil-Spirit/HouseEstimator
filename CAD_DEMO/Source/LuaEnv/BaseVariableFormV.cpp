...
void __fastcall TGlobal::ListDblClick(TObject *Sender)
{
    ...
    if (Item)
    {
        ...
        Var->Value=InputBox("Изменение переменной", "Введите новое значение",Item->SubItems[0][1]);
        FileMap->Position=0;
...
void __fastcall TGlobal::EditItemExecute(TObject *Sender)
{
    ...
    AnsiString Code = InputBox("Editing watch","Введите watch:",Item->Caption);
    if (Code!="")
    {
        Item->Caption=Code;
...
{
    AnsiString Code = InputBox("Adding watch","Введите watch:","");
    if (Code!="")
    {
        TListItem *Item = List->Items->Add();
        Item->Caption=Code;
        Item->SubItems->Add("{not avaible}");
        Evaluate(Sender);
    }
}
//---------------------------------------------------------------------------
...
void AddWatch(AnsiString Data)
{
    TQuickList *Lst = new TQuickList();
    SelectComponents(__classid(TGlobal),Lst);
    for (int i=0;i<Lst->Count;i++)
    {
        TGlobal *Wnd = (TGlobal*)Lst->Items[i];
        if (Wnd->Caption=="Watches")
        {
            TListItem *Item = Wnd->List->Items->Add();
            Item->Caption=Data;
            Item->SubItems->Add("{not avaible}");
            Wnd->Evaluate(Wnd);
        }
