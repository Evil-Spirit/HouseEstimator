...
type
  ...
  protected
    ...
    property ComboBox: TMyCustomCombo read FComboBox write FComboBox;
  public
  ...
  TMyCustomCombo = class(TCustomListControl)
  private
    FCanvas: TCanvas;
    FMaxLength: Integer;
  ...
  TCustomComboBox = class(TMyCustomCombo)
  private
...
constructor TMyCustomCombo.Create(AOwner: TComponent);
const
...
destructor TMyCustomCombo.Destroy;
begin
  if HandleAllocated then
    DestroyWindowHandle;
...
procedure TMyCustomCombo.Clear;
begin
  SetTextBuf('');
  FItems.Clear;
  FSaveIndex := -1;
...
procedure TMyCustomCombo.DestroyWindowHandle;
begin
...
procedure TMyCustomCombo.SelectAll;
begin
  SendMessage(Handle, CB_SETEDITSEL, 0, Integer($FFFF0000));
end;

function TMyCustomCombo.GetDroppedDown: Boolean;
begin
  Result := LongBool(SendMessage(Handle, CB_GETDROPPEDSTATE, 0, 0));
end;

procedure TMyCustomCombo.SetDroppedDown(Value: Boolean);
var
  R: TRect;
begin
...
function TMyCustomCombo.GetItemIndex: Integer;
begin
...
procedure TMyCustomCombo.SetItemIndex(const Value: Integer);
begin
  if csLoading in ComponentState then
    FItemIndex := Value
  else
...
function TMyCustomCombo.GetSelStart: Integer;
begin
  SendMessage(Handle, CB_GETEDITSEL, Longint(@Result), 0);
end;

procedure TMyCustomCombo.SetSelStart(Value: Integer);
var
  Selection: TSelection;
begin
...
function TMyCustomCombo.GetSelLength: Integer;
var
  Selection: TSelection;
begin
...
procedure TMyCustomCombo.SetSelLength(Value: Integer);
var
  Selection: TSelection;
begin
  SendMessage(Handle, CB_GETEDITSEL, Longint(@Selection.StartPos),
...
procedure TMyCustomCombo.SetMaxLength(Value: Integer);
begin
...
procedure TMyCustomCombo.SetItemHeight(Value: Integer);
begin
  if Value > 0 then
  begin
    FItemHeight := Value;
...
procedure TMyCustomCombo.WMCreate(var Message: TWMCreate);
begin
  inherited;
...
procedure TMyCustomCombo.WMDrawItem(var Message: TWMDrawItem);
begin
  DefaultHandler(Message);
end;

procedure TMyCustomCombo.WMMeasureItem(var Message: TWMMeasureItem);
begin
  DefaultHandler(Message);
end;

procedure TMyCustomCombo.WMDeleteItem(var Message: TWMDeleteItem);
begin
  DefaultHandler(Message);
end;

procedure TMyCustomCombo.WMGetDlgCode(var Message: TWMGetDlgCode);
begin
  inherited;
  if DroppedDown then Message.Result := Message.Result or DLGC_WANTALLKEYS;
end;

procedure TMyCustomCombo.CMCancelMode(var Message: TCMCancelMode);
begin
  if Message.Sender <> Self then Perform(CB_SHOWDROPDOWN, 0, 0);
end;

procedure TMyCustomCombo.CMCtl3DChanged(var Message: TMessage);
begin
...
procedure TMyCustomCombo.EditWndProc(var Message: TMessage);
var
  P: TPoint;
  Form: TCustomForm;
begin
...
procedure TMyCustomCombo.ListWndProc(var Message: TMessage);
begin
...
procedure TMyCustomCombo.ComboWndProc(var Message: TMessage; ComboWnd: HWnd;
  ComboProc: Pointer);
var
  Point: TPoint;
  Form: TCustomForm;
...
procedure TMyCustomCombo.WndProc(var Message: TMessage);
begin
...
procedure TMyCustomCombo.CNCommand(var Message: TWMCommand);
begin
...
procedure TMyCustomCombo.Change;
begin
  inherited Changed;
  if Assigned(FOnChange) then FOnChange(Self);
end;
...
procedure TMyCustomCombo.DropDown;
begin
  if Assigned(FOnDropDown) then FOnDropDown(Self);
end;

procedure TMyCustomCombo.Loaded;
begin
  inherited Loaded;
...
function TMyCustomCombo.Focused: Boolean;
var
  FocusedWnd: HWND;
begin
  Result := False;
...
procedure TMyCustomCombo.CloseUp;
begin
  if Assigned(FOnCloseUp) then
    FOnCloseUp(Self);
end;

procedure TMyCustomCombo.Select;
begin
...
procedure TMyCustomCombo.CreateWnd;
begin
  inherited CreateWnd;
...
procedure TMyCustomCombo.AdjustDropDown;
var
  Count: Integer;
begin
  Count := ItemCount;
...
procedure TMyCustomCombo.SetItems(const Value: TStrings);
begin
...
procedure TMyCustomCombo.ClearSelection;
begin
  ItemIndex := -1;
end;

procedure TMyCustomCombo.CopySelection(Destination: TCustomListControl);
begin
  if ItemIndex <> -1 then
    Destination.AddItem(Items[ItemIndex], Items.Objects[ItemIndex]);
end;

procedure TMyCustomCombo.DeleteSelected;
begin
  if ItemIndex <> -1 then
    Items.Delete(ItemIndex);
end;

function TMyCustomCombo.GetCount: Integer;
begin
  Result := GetItemCount;
end;

procedure TMyCustomCombo.SetDropDownCount(const Value: Integer);
begin
  FDropDownCount := Value;
end;

procedure TMyCustomCombo.AddItem(Item: String; AObject: TObject);
begin
