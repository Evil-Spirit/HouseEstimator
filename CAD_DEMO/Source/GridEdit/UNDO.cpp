...
//ñòðîãîå òðåáîâàíèå:
//íåâîçìîæíà ðåãèñòðàöèÿ äâàæäû
//èñêëþ÷åíèå ïîâòîðíàÿ ðåãèñòðàöèÿ î óäàëåíèè
...
void TGridUndoStack::OpenSession(const AnsiString& SessionName)
{
    ...
    {
        WarningMsg("<TGridUndoStack::OpenSession>: Ñåññèÿ ñòåêà èçìåíåíèé óæå îòêðûòà");
        return;
    }
    RedoStack.Clear();
...
void TGridUndoStack::CloseSession()
{
    if (Sessions.Count && Sessions.Last()->Closed)
    {
        WarningMsg("<TGridUndoStack::CloseSession>: Ñåññèÿ ñòåêà èçìåíåíèé óæå çàêðûòà");
        return;
    }
    if (Sessions.Count)
        Sessions.Last()->Close();
...
void TGridUndoStack::BreakSession()
{
    if (Sessions.Count && Sessions.Last()->Closed)
    {
        WarningMsg("<TGridUndoStack::BreakSession>: Ñåññèÿ ñòåêà èçìåíåíèé óæå çàêðûòà");
        return;
    }
    if (Sessions.Count)
...
void TGridUndoStack::Undo()
{
    _TRY_
        Direct_Undo();
    _ENDTRY_("<TGridUndoStack::Undo>: Îòìåíà íå âûïîëíåíà",);
}
...
void TGridUndoStack::Redo()
{
    _TRY_
        Direct_Redo();
    _ENDTRY_("<TGridUndoStack::Redo>: Âîçâðàò îòìåíû íå âûïîëíåí",);
}
