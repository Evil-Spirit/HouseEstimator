...
void   TCellGrid::DrawTextU2(int CurCol, int CurRow, const TRect& RectDrawText)// Ðèñîâàíèå òåêñòà
{
            ...
            if ((Where.Right > Where.Left) && RectVisible(Canvas->Handle, &Where))
            {
                ...
                )
                {
                    if ((DefaultDrawing)||(ComponentState.Contains(csDesigning)))
                    {
                        ...
                        ColorSelect =(TColor)RGB(109,149,213); // Âûäåëåíèå íåñêîëüêèõ ÿ÷÷åê
                        if (MoveCellBorder && !PointInRect(CurCol,CurRow,TRect(StartMouseDown.X,StartMouseDown.Y,MouseUpCoord.X,MouseUpCoord.Y)))
                            ColorSelect =(TColor)RGB(255,255,255); // Âûäåëåíèå íåñêîëüêèõ ÿ÷÷åê
                        ...
                        if (StartVisible)
                        {
                            ...
                            ::DeleteObject(MyRgn);
//---------- Îáëàñòü âñåõ ÿ÷ååê (ïóñòûõ, ñîçäàííûõ) ---------------------------
// Ðèñóåò ëèíèè íà ïåðâûõ ÿ÷åéêà
                            ...
                            SetNumerColRow(CurCol, CurRow,1, Canvas->Brush->Color);// Íóìåðàöèÿ ïåðâûõ ÿ÷ååê
//-------------------------------- END ---------------------------------
                        }
                    }
                }
