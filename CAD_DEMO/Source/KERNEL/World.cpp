...
{
    /* TODO : îòêëþ÷åíèå íåïðàâèëüíîãî ýëåìåíòà */
//    Select->ErrElement = NULL;
    ...
    for (int i=0;i<L->Count;i++)
        ...
        {
            /* TODO : âêëþ÷åíèå íåïðàâèëüíîãî ýëåìåíòà */
...
//recurrent check   àãðåãàòíàÿ èåðàðõèÿ tmyobject ()
// check data èåðàðõèÿ â äåðåâå
bool TMainTree::Init()
{
...
{
//    AUI.UnRegister(this);    
//  /* TODO : îòêëþ÷åíèå äåéñòâèÿ îáðàòèòü âíèìàíèå */  Select->Terminate();
    Clear();
    delete FloorInfo;
    delete SERVICE;
    if (World == this)
...
void TMainTree::RecurrentSelectByElementLinks(TUnit *Node,TMetaElement *ME_B,TMTList<TElement> *List)
{
    ...
    if ( Node->Is(TElement::StaticType) )
    {
        ...
        //âûãðóæàþòñÿ â ñïèñîê âñå îáúåêòû
        //äàæå ñ íåîáÿçàòåëüíûìè ñâÿçêàìè
...
bool TMainTree::AddFloor(int index)
{
    ...
    //åñëè äîáàâëÿåòñÿ ýòàæ â ñåðåäèíó òî ïîäíÿòü ýòàæè
