...
TElement::TElement()
{
    ...
    IntId = TotalCount++;
    //ðåàëüíî ìû íå çíàåì áóäåò ëè ðåãèòñÿ ýëåìåíò
    //ïîýòîìó êîìåíòàðèì UndoRegister(rmCreated);
    ATVChanged = true;
...
void TElement::ProcessCreateView()
{
  ...
  //-----------------------------
    ...
    if (!RunElementTrigger(trConstruct))
        StandartCreateView();
    /* TODO : Ïðîâåðêà íà âõîæäåíèå â Select, ÷òîáû àïäåéòèòü åãî */        
    /*
...
void TElement::ProcessUpdatePos()
{
    ...
    /* TODO : Ïðîâåðêà íà âõîæäåíèå â Select, ÷òîáû àïäåéòèòü åãî 2 */
    /*if (Select->Element == this)
    {
...
TElement::~TElement()
{
    ...
    //Íåëüçÿ ýòîãî äåëàòü ò.ê. ðåàëüíûå ïîëÿ â â äî÷. îáúåêòîàõ óæå ñäîõëè 
    // ïîýòîìó êîìåíòèðóåì UndoRegister(rmDeleted);
    ...
    //TaskQueue.UnRegister(this); // íå íóæíî ò.ê. ñëåäóþùàÿ ñòðîêà
...
{
    FLinks.Add(Link);
    //Ñâÿçêà ñàìà ñåáÿ äîáàâëÿåò
}

int TElement::GetNeededLink()
{
    //return this
    //ýòó ôóíêöèþ ïåðåäåëàòü ÷òîáû áûë èíäåêñ
    int Needed=-1;
    int NeededCount=0;
    for (int i=0;i<FLinks.Count;i++)
...
void TElement::KillLink(void *Sender,int Index)
{
    ...
    if (NeededLink==-1)
    {
    //------------------ïîïàäàåò ëè â äðàéâåð----------------------------------
        Parent = NULL;
        if (MyTree->Head==this)//Ó êîðíÿ îáÿçàòåëüíûõ ñâÿçîê íåò
            return;
...
{
   /* TODO : Ïðîâåðêà íà ðåäàêòèðîâàíèå - ðåäàêòèðóåòñÿ ëè ýëåìåíò ñåé÷àñ */ 
/*    if (BIF->Element != this)
    {
        Operation = ioNone;
        Type = 0;
    ...
    TUnit()
{
    ...
    //------------------------------
    //Çäåñü ìû íå çíàåì áóäåò ëè ðåãèñòðèðîâàòüñÿ
    //ïîýòîìó êîììåíòèðóì UndoRegister(rmCreated);
