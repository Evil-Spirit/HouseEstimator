...
uses
  ...
  PictureEditorU, StringListEditorU;
...
type PropIdent = (piSet, piSetElement, piClass, piEnumeration, piPointer, piNone);
...
procedure SetPropByKind (Kind: TTypeKind; _Object: TObject; PropName: String; PropValue: Variant; PropObject: TObject = nil);
function PropertyCount(_Object: TObject; var PropCount, MethodCount: Integer ) : Integer;

var
  ObjectInspectorF: TObjectInspectorF;
                ...
                begin
                  ...
                  if TempObject <> nil then
                    begin
                      ...
                      if PropertyList[PropIndex].PropertyType.Name = 'TStrings' then
                        begin
                          PropertyList[PropIndex].Lines := TStrings.Create;
                          PropertyList[PropIndex].Lines.BeginUpdate;
                          PropertyList[PropIndex].Lines.EndUpdate;
                        end;
                    end;
                end;
              tkSet:
...
{----------------------------------------------------------------------------}
procedure SetPropByKind (Kind: TTypeKind; _Object: TObject; PropName: String; PropValue: Variant; PropObject: TObject);
begin
  case Kind of
                    ...
                    begin
                      ...
                      if TempValue <> '' then
                        if SelectedObjects[i].PropertyList[j].ThisObject <> nil then
                          SetPropByKind(TempProp.FPropertyKind, SelectedObjects[i].FObject, SelectedObjects[i].PropertyList[j].FPropertyName, SelectedObjects[i].PropertyList[j].FPropertyValue, TPropObject(SelectedObjects[i].PropertyList[j].ThisObject).FObject)
                        else
                          SetPropByKind(TempProp.FPropertyKind, SelectedObjects[i].FObject, SelectedObjects[i].PropertyList[j].FPropertyName, SelectedObjects[i].PropertyList[j].FPropertyValue);
                    end
                  else
                    begin
                            ...
                            begin
                              ...
                              if TempValue <> '' then
                                if TempProp.ThisObject <> nil then
                                  SetPropByKind(TempProp.FPropertyKind, TempParent.FObject, TempParent.PropertyList[j].FPropertyName, TempParent.PropertyList[j].FPropertyValue, TPropObject(TempParent.PropertyList[j].ThisObject).FObject)
                                else
                                  SetPropByKind(TempProp.FPropertyKind, TempParent.FObject, TempParent.PropertyList[j].FPropertyName, TempParent.PropertyList[j].FPropertyValue);
                            end;
                        end;
                    end;
...
begin
  ...
  BeginIndex := Row;
  for i := 0 to PropertyValueListEditor.Height div (PropertyValueListEditor.RowHeights[0] + 1) do
...
//            ÑolorBox.Left := 1;
//            ColorBox.Right := 10;
//            ColorBox.Top := 1 + Index * (PropertyValueListEditor.RowHeights[0] + 1) + (PropertyValueListEditor.RowHeights[0] + 1) div 2 - 5;
//            ColorBox.Bottom := 10 + Index * (PropertyValueListEditor.RowHeights[0] + 1) + (PropertyValueListEditor.RowHeights[0] + 1) div 2 - 5;
...
begin
  ...
  FDrawFlag := 0;
  OIComboBoxChange(ObjectInspectorF);
  DefValue.Free;
end;
...
begin
  ...
  if ( PropertyValueListEditor.Selection.Left = 1 ) and ( TempProp.PropertyName = 'Font' ) then
    begin
      ...
      MySelection.Top := MySelection.Top - 7;
      MySelection.Bottom := MySelection.Bottom - 7;
      PropertyValueListEditor.Selection := MySelection;
      Exit;
    end;
  ...
  if ( PropertyValueListEditor.Selection.Left = 1 ) and ( TempProp.PropertyType.Name = 'TStrings' ) then
    begin
      ...
      for j := 0 to StringListEditor.MainMemo.Lines.Count - 1 do
        StringListEditor.MainMemo.Lines.Strings[i] := TempProp.Lines[i];
