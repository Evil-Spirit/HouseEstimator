...
{
	TIntVec l = (B-A);
    l.z = 0;//ïîääåðæêà íà ñëó÷àé òî÷åê íå íà ïëîñêîñòè
...
//	throw EMyException("DistancePointCut - êðèòè÷åñêàÿ îøèáêà!");
	return 0;
}
...
/*!!!!!!!!!!!!!!!
  ...
  !! 28.03.2005!!
  !!!!!!!!!!!!!!!*/
int PlaneSegmentCross(	const	TIntVec& N,	//íîðìàëü
						const	TIntVec& A,	//òî÷êà íà ïëîñêîñòè

						const	TIntVec& X,	//ïðÿìàÿ òî÷êà 1
						const	TIntVec& Y,	//ïðÿìàÿ òî÷êà 2

						bool	xFix,		//ïîïàäàíèå íà÷àëà
						bool	yFix,		//ïîïàäàíåèå êîíöà
						...
						MBTi USEEPS)    	//ìåðà ïàðàëëåëüíîñòè
{
	...
	/*TODO: Ïðîâåðèòü äåëåíèå íà íîëü*/
...
/*!!!!!!!!!!!!!!!
  ...
  !! 28.03.2005!!
  !!!!!!!!!!!!!!!*/
int  PlaneCutCrossClass(	const TIntVec& N,//òî÷êà íà ïëîñêîñòè
							const TIntVec& A,//íîðìàëü
							const TIntVec& X,//ïðÿìàÿ òî÷êà 1
							const TIntVec& Y,//ïðÿìàÿ òî÷êà 2
							TIntVec& O, //òî÷êà ïåðåñå÷åíèÿ
							MBTi USE_PEPS)    //ìåðà ïàðàëëåëüíîñòè
{
	// îïðåäåëèòü ôàêò ïåðåñå÷åíèÿ îòðåçêà è ïëîñêîñòè.
...
/*!!!!!!!!!!!!!!!
  ...
  !! 28.03.2005!!
  !!!!!!!!!!!!!!!*/
int PlaneLineCross(	const	TIntVec& N,	//íîðìàëü
					const	TIntVec& A,	//òî÷êà íà ïëîñêîñòè
...
/*!!!!!!!!!!!!!!!
  ...
  !! 28.03.2005!!
  !!!!!!!!!!!!!!!*/
int PlaneCutCross(	const	TIntVec& N,	//íîðìàëü
					const	TIntVec& A,	//òî÷êà íà ïëîñêîñòè
...
bool PlaneCrossPoint(   const TIntVec& A,//òî÷êà íà ïëîñêîñòè
...
/*	if ( _P00.x == _P10.x &&  _P00.y == _P10.y && _P01.x == _P11.x && _P01.y == _P11.y )
	...
	MBTi dx1 = _P01.x-_P00.x, dy1 = _P01.y-_P00.y; // Äëèíà ïðîåêöèé ïåðâîé ëèíèè íà îñü x è y
	MBTi dx2 = _P11.x-_P10.x, dy2 = _P11.y-_P10.y; // Äëèíà ïðîåêöèé âòîðîé ëèíèè íà îñü x è y
	MBTi dxx = _P00.x-_P10.x, dyy = _P00.y-_P10.y;
	MBTi div, mul;

	if (  fabs(div = (dy2*dx1-dx2*dy1)) < _MBTi_eps_ )
	{
		return (lcPARALLEL); // Ëèíèè ïàðàëëåëüíû...
	}
...
/*
    ...
    if (min0.x > max1.x || max0.x < min1.x || min0.y > max1.y || max0.y < min1.y)
        return lcNONE;  // Ìîìåíò, êàäà ëèíèè èìåþò îäíó îáùóþ âåðøèíó...

    long double dx1 = P01.x-P00.x, dy1 = P01.y-P00.y; // Äëèíà ïðîåêöèé ïåðâîé ëèíèè íà îñü x è y
    long double dx2 = P11.x-P10.x, dy2 = P11.y-P10.y; // Äëèíà ïðîåêöèé âòîðîé ëèíèè íà îñü x è y
...
/*    if ( fabs(_P00.x-_P10.x)<=USEEPS &&  fabs(_P00.y-_P10.y)<=USEEPS && fabs(_P01.x-_P11.x)<=USEEPS && fabs(_P01.y-_P11.y)<=USEEPS )
	...
	if (div > USEEPS)
	{
		...
		if ( mul  < -USEEPS || mul > div+USEEPS )
			return lcNONE; // Ïåðâûé îòðåçîê ïåðåñåêàåòñÿ çà ñâîèìè ãðàíèöàìè...
		mul = dx2*dyy-dy2*dxx;
		if ( mul  < -USEEPS || mul > div+USEEPS )
			return lcNONE; // Âòîðîé îòðåçîê ïåðåñåêàåòñÿ çà ñâîèìè ãðàíèöàìè...
	}
	else
	{
		mul = -(dx1*dyy-dy1*dxx);
