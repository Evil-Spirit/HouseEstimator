...
bool T2DGObject::Snap(const TIntVec &Point, TIntVec& SnapPoint, int& CutIndex, const MBTi CEPS, bool Infinity) const
{
    ...
    for (int i = 0; i < CutsCount; i++)
    {
        ...
        {
            d = sqrt(fabs(x-Point.x)*fabs(x-Point.x)+fabs(y-Point.y)*fabs(y-Point.y));
            if (d < minCEPS)
            {
                minCEPS = d;
                minIndex = i;
...
TG2DLink* T2DGObject::MakeAngleLink(TG2DCut* Cut1,TG2DCut* Cut2,TAngleLinkSector aAngleLinkSector)
{
    for ( int i=0;i<FLinks.Count;i++ )
        if (    FLinks[i].Is( TAngleLink::StaticType ) &&
            ...
            throw EMyException("<T2DGObject::MakeAngleLink>: the link is already exist.");
...
TG2DLink* T2DGObject::MakeDistanceLink(TG2DPoint* Point1,TG2DPoint* Point2)
{
    for ( int i=0;i<FLinks.Count;i++ )
        if ( FLinks[i].Is( TDistanceLink::StaticType ) &&
            ...
            throw EMyException("<T2DGObject::MakeDistanceLink>: the link is already exist.");

    FLinks.Add( CreateDistanceLink(Point1,Point2) );
    return FLinks.Last();
}

void T2DGObject::Clear() // Î÷èñòèòü ãåîìåòðè÷åñêèé îáüåêò
{
    FLinks.Clear();
    TGeomObject::Clear();
}
...
void T2DGObject::SelectGroupForPoint(TG2DPoint* Point,TFlowInfo& FlowInfo)
{
    ...
    while (WasAdded)
    {
        ...
        for ( int i=0;i<FVertex.Count;i++ )
            if ( FVertex.Items[i]->Flags.Mark != gfTRUE  )
                if (  IsLinkedTo(FlowInfo.PntList,(TG2DPoint*)FVertex.Items[i],true, TMTList<TG2DLink>() )  )
                {
                    FlowInfo.PntList.Add( (TG2DPoint*)FVertex.Items[i] );
    ...
    //--------------------------------------------------------------------------
    for ( int i=0;i<FLinks.Count;i++ )
        if ( LinkOnTheSet( FlowInfo.PntList,FLinks.Items[i] ) )
            FlowInfo.Links.Add( FLinks.Items[i] );
    //--------------------------------------------------------------------------
    for (int i=0;i<FCuts.Count;i++)
        if (FlowInfo.PntList.IndexOf( (TG2DPoint*)FCuts[i].Src ) != -1 &&
            FlowInfo.PntList.IndexOf( (TG2DPoint*)FCuts[i].Dst ) != -1 )
            FlowInfo.AllCuts.Add( (TG2DCut*)FCuts.Items[i] );
}
...
{
    if (pFlowInfo)
        throw EMyException("<T2DGObject::PrepareMoving>: òåêóùàÿ îïåðàöèÿ ïîçèöèîíèðîâàíèÿ íå çàâåðåøåíà.");
    pFlowInfo = new TFlowInfo();
    pFlowInfo->Owner = this;
    pFlowInfo->Point = Point;
    SelectGroupForPoint(Point,*pFlowInfo);
    CreateInfluence(*pFlowInfo,Point);
}
...
{
    MakeTempPoints();
    pFlowInfo->Point->Point = NewValue;
    bool Result = true;
