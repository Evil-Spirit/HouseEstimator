...
bool FindAngleEdgesRec(TG2DCut* NotCut,TG2DCut* Cut,TFlowInfo& FlowInfo,int index)
{
    ...
    for (int i=0;i<Cut->LinksCount;i++)
        if ( Cut->GetLink(i).Is( TAngleLink::StaticType ) )
        {
            ...
            {
                ErrorMsg("<FindAngleTreeRec>: Sketch redefined.");
                return false;
            }
            bool res = FindAngleEdgesRec(Cut,ElseCut,FlowInfo,index);
            if (!res)
                return false;
        }
...
bool FindAngleEdges(TFlowInfo& FlowInfo)
{
    ...
    while (ElseCuts.Count)
    {
        ...
        bool res = FindAngleEdgesRec(NULL,ElseCuts.Items[0],FlowInfo,FlowInfo.EdgeCuts.Add( new TMTList<TG2DCut> ));
        if (!res)
            return false;
        for ( int i=0;i<ElseCuts.Count;i++ )
    ...
    for (int i=0;i<FlowInfo.EdgeCuts.Count;i++)
        for (int j=0;j<FlowInfo.EdgeCuts[i].Count;j++)
            ...
            {
                TG2DPoint* GPoint = (TG2DPoint*)FlowInfo.EdgeCuts[i][j].CanConnect( FlowInfo.EdgeCuts[i][k] );
                if (!GPoint)
                    continue;
                for (int l=0;l<FlowInfo.Skeletons.Count;l++)
                    if ( FlowInfo.Skeletons[l].IndexOf(GPoint) != -1)
                        if ( FlowInfo.EdgeCutSkeletons.IndexOf(&FlowInfo.Skeletons[l]) == -1 )
                            FlowInfo.EdgeCutSkeletons.Add(&FlowInfo.Skeletons[l]);
            }
    return true;
}
...
bool Ensure_Not_Skeleton_Links(TG2DPoint* GPoint,TFlowInfo& FlowInfo)
{
    ...
    while (IsFindedLink)
    {
        ...
        for ( int i=0;i<FlowInfo.NotSkeletonLinks.Count;i++ )
        {
            ...
            if (NewGPoint)
            {
                ...
                {
                    ErrorMsg("<ProcessAddPointToSequence>: Sketch redefined.");
                    return false;
                }
    ...
    //åñëè <îáå òî÷êè ïðèíàäëåæàò îäíîìó ñêåëåòîíó> È <÷èñëî ñòåïåíåé ñâîáîäû ñêåëåòîíà == îäíà> òîãäà
        // ñîçäàåì ñâÿçêó
    for (int i=0;i<FlowInfo.NotSkeletonLinks.Count;i++)
    {
        ...
        for (int i=0;i<FlowInfo.MSPS.Count;i++)
            for (int j=0;j<FlowInfo.Skeletons[i].Count;j++)
                if (    FlowInfo.Skeletons[i][j].Rules.Count<2 &&
                        ...
                        !CheckPointOnEdgeCuts (&FlowInfo.Skeletons[i][j],FlowInfo) ëèøíåå ò.ê. ñâÿçîê óæå íåò*/ )
