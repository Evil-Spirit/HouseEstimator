...
{
    //-= 1-ÿ áàçîâàÿ òî÷êà
    for (int i=0; i<FVertex.Count; i++)  //åùå óñëîâèå ïî x
...
{
    if ( !p1 || !p2 )
		throw EMyException ("<TGeomObject::NewCut> íåâîçìîæíî äîáàâèòü îòðåçîê!");
...
void TGATriangulate::TriangulateByPoints()
{
    ...
    if ( P2 == NULL )
        throw EMyException("<TGATriangulate::TriangulateByPoints> Ïîñëå Find2BasePoints P2 == NULL");
