...
TPolygon::TPolygon(TMDelTList<TIntVec>* PNTS)
{
    ...
    //ðåãèñòðàöèÿ çäåñü íåëîïóñòèìà èç-çà ïåðåàäðåñàöèè ñîîáùåíèé
...
{
//    if (NotInstanced)
  //      throw EMyException("<TPolygon::Clear()>: Ïîëèãîí is read only!");
    Vertex->Clear();
}
...
{
//    if (NotInstanced)
  //      throw EMyException("<TPolygon::KillCloseEdges_0()>: Ïîëèãîí is read only!");
    TMDelTList<TIntVec>& vVertex = *(Vertex);
    for (int i=0;i<vVertex.Count;i++)
...
{
//    if (NotInstanced)
  //      throw EMyException("<TPolygon::KillCollinear_1()>: Ïîëèãîí is read only!");
    TMDelTList<TIntVec>& vVertex = *(Vertex);
    if (vVertex.Count<3)
        return false;
    for (int i=0;i<vVertex.Count;i++)
...
{
//    if (NotInstanced)
  //      throw EMyException("<TPolygon::SquareCreate()>: Ïîëèãîí is read only!");
    Clear();
...
{
//    if (NotInstanced)
  //      throw EMyException("<TPolygon::Valid()>: Ïîëèãîí is read only!");
...
int TPolygon::ConsistsPolygon(TPolygon* ElsePol)
{
    ...
    if (CountOUTSIDE)
        return(p_pOUTSIDE);
    //âñå òî÷êè ãðàíè÷íûå
    //òåïåðü íàäî ïðîâåðèòü îäíó âíóòðåííþþ òî÷êó Elsepol
...
{
    if (NotInstanced)
        throw EMyException("<TPolygon::Assign()>: Ïîëèãîí is read only!");
...
{
//    if (NotInstanced)
  //      throw EMyException("<TPolygon::IntrudeRib()>: Ïîëèãîí is read only!");
    TMDelTList<TIntVec>& vVertex = *Vertex;
    if (vVertex.Count<3)
        return psNoPoints;
...
{
//    if (NotInstanced)
  //      throw EMyException("<TPolygon::Move()>: Ïîëèãîí is read only!");
    for (int i=0;i<Vertex->Count;i++)
...
{
//    if (NotInstanced)
  //      throw EMyException("<TPolygon::Rotate()>: Ïîëèãîí is read only!");
    for (int i=0;i<Vertex->Count;i++)
...
{
//    if (NotInstanced)
  //      throw EMyException("<TPolygon::Insert()>: Ïîëèãîí is read only!");
...
{
//    if (NotInstanced)
  //      throw EMyException("<TPolygon::Set()>: Ïîëèãîí is read only!");
    for (int i=0;i<Vertex->Count;i++)
...
{
//    if (NotInstanced)
  //      throw EMyException("<TPolygon::ToConvex()>: Ïîëèãîí is read only!");
    bool WasDelete = true;
    while (WasDelete)
    {
        if (Vertex->Count<=3)
...
{
//    if (NotInstanced)
  //      throw EMyException("<TPolygon::ToConvexAndExtrude()>: Ïîëèãîí is read only!");
    ToConvex(USEEPS);
    if (Vertex->Count<3)
        return;
    TMDelTList<TIntVec>& Vert = *Vertex;
                                                ...
                                                MBTi USEEPS)
{
    ...
    for (int i=0;i<Vert.Count;i++)
        ...
        {
            //åñëè ðàâíû òî îáùèå òî÷êè  â i è â i+1
            case lcEQUAL:
                continue;
            case lcCOMMONPOINT:
            case lcCROSSING:
    ...
    //ðàçâåðíåì ñïèñîê â èçíà÷àëüíîå ïîëîæåíèå
//    if ( (PList[0]-P1).Length2D() > (PList[PList.Count-1]-P1).Length2D() )
//        PList.Invert();

    //ïîñìîòðèì íà ïðèíàäëåæíîñòü 1 îòðåçîê ïðèíàäëåæèò -1 íåò 0 íåèçâåñòíî
