...
bool TGAFix::Repeated(TMTList<TGCut> &cuts, bool atall) //TODO: ñäåëàòü ïîëíîå óäàëåíèå ïîâòîðíûõ ïî ôëaãó
{
	bool ret = false;
	for (int i=0;i<cuts.Count;i++)
	...
	for (int i=0;i<p->FCuts.Count;i++)
		for (int j=0;j<p->FCuts[i].FPlane.Count;j++)
			if (p->FCuts[i].FPlane.Items[j] && p->FCuts[i].FPlane.Items[j] != p)
			{
				...
				if (!pCut || !oCut)
					throw EMyException("<SynchronizePolygonsWisesRecursive> : ¢ªþ-ªþ ýõ ªðú!");
