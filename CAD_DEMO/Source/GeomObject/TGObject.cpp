...
TGCut *TGeomObject::AddCut(TGPoint *p1, TGPoint *p2)
{
	if (CheckCutData && (FVertex.IndexOf(p1) == -1 || FVertex.IndexOf(p2) == -1 || p2 == p1))
		throw EMyException ("<TGeomObject::AddCut> îøèáêà äàííûõ!");
		...
		if (c->FPlane.Items[i])
		{
			...
			if (c->FPlane[i].FCuts.CycVal(index-1).CanConnect(*cn))
			{
				...
				if (c->FPlane[i].FCuts.IndexOf(cn1)!=-1 || c->FPlane[i].FCuts.IndexOf(cn)!=-1)
					throw EMyException("<TGeomObject::SplitCut> - ýðûþöõýøõ þª¨õ÷úþò!");
#endif				
...
{
	if (!c)
		throw EMyException("<TGeomObject::MultiSplitCut> : NULL îòðåçîê!");
...
/*	if (!n || !n2)
	...
	if (n2->DynamicType->Name != TGCut::StaticType->Name ||
		...
		throw EMyException("<TGeomObject::UpdateCutPointers> :: Íåñîâòàäåíèå òèïà!"); 
*/		
	for (int i=0;i<FUserCuts.Count;i++)
	{
		int ind;
...
bool TGeomObject::SplitPolygon(TGPolygon *p, TMTList<TGCut> &Ct, TMTList<TGPolygon> &Result)
{
	...
	TMTList <TGCut> Heap;			// Êó÷à îòðåçêîâ
	TMTList <TGPoint> polyPnt;		// Òî÷êè ïîëèíîíà
	...
	// Îòìå÷àåì ïîëèãîíû äûêàìè, èñõîäÿ èç òîãî, ÷òî ïîëèãîíû ïðåíàäëåæàò äûðêàì. Ìîæíî îïòèìèçèðîâàòü, âñòàâèâ TGPolygon::Equals
	...
	// Ñòðîèì ìàòðèöó ïðèíàäëåæíîñòåé
	TMDelTList<TMDelTList<int> > matrix;
