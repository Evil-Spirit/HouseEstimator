...
{
    if ( index<0 || index>1 )
        throw EMyException(AnsiString("<TGBaseLink::SetClient>: íåâåðíûé èíäåêñ êëèåíòà ñâÿçêè - ") + IntToStr(index) );
...
TGUnit* TGBaseLink::AnotherClient(TGUnit* aClient)
{
    ...
    throw EMyException("<TGUnit::AnotherClient>: Êëèåíò íå ïðèíàäëåæèò ñâÿçêå.");
}
