...
void TLList<T>::SetCount(int NewCount)
{
	if ( NewCount < 0 )
	{
		...
		STR = STR + AnsiString("Îøèáêà ïðè èçìåíåíèè êîë-âà ýëåìåíòîâ ñïèñêà, êîë-âî - ");
...
void TLList<T>::SetCapacity(int NewCapacity)
{
	...
	{
		AnsiString STR = AnsiString("<")+DynamicType->Name+AnsiString(">: ");
		STR = STR + AnsiString("Îøèáêà ïðè óâåëè÷åíèè ðàçìåðà ñïèñêà, òåêóùèé ðàçìåð - ");
		STR = STR + IntToStr(FCount)+AnsiString(", òðåáóåìûé ðàçìåð - ");
...
bool TLList<T>::Direct_IndexOK(int i) const
{
	...
	{
		AnsiString STR = AnsiString("<")+DynamicType->Name+AnsiString(">: ");
		STR = STR+ AnsiString("Index is out of range ñïèñêà, Ðàçìåð - ");
		STR = STR+IntToStr(FCount)+AnsiString(", Èíäåêñ - ");
		STR = STR+IntToStr(i);
		throw EMyException(STR);
	}
}
