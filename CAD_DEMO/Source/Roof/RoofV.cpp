...
MBTi PEPS = 0.0001;				//ìåðà áëèçîñòè òî÷åê
MBTi CEPS = 0.0001;				//ìåðà êîëëèíåàðíîñòè ïëîñêîñòåé
MBTi EEPS = 0.0001;				//îáùàÿ ìåðà áëèçîñòè 
MBTi ZERO_EPS = 0.000001;       //îáùàÿ ìåðà áëèçîñòè
...
void TCollinearGroup::InsertPoint(TMDelTList<int> &PntPlane, TGPoint* Pnt,MBTi USEEPS)
{
	...
	//òåïåðü ïðîâåðèì ìîæåò òî÷êà ëåæèò íà ïðÿìîé
	//îáîçíà÷àþùåé ãðàíèöó êîëëèíåàðíîñòè
	for (int i=0;i<Edges.Count;i++)
		if (Edges[i].COMMON.IndexOf(Pnt)!=-1)
		{
			//åñëè äà òî äîáàâëÿåì ÄÂÅ ïëîñêîñòè â ñïèñîê ïëîñêîñòåé
			//÷åðåç êîòîðûå ïðîõîäèò òî÷êà
			PntPlane.Add(new int(indices[i]) );
			PntPlane.Add(new int(indices[i+1]) );
			return;
		}
	//òî÷êà íå ëåæèò íà ãðàíèöå
	//òåïåðü îäíîçíà÷íî îòíåñåì åå ê îäíîé èç ïëîñêîñòåé
	//â ñìûñëå çàäàííûõ ãðàíèö êîëëèíåàðíîñòè
	TIntVec Intersect;
...
{
		//ðàñ÷èòûâàåì òî÷êè ïåðåñå÷åíèÿ  ó ïñåâäîòðåóãîëüíèêîâ
								...
								TIntVec& b_O)
{
	...
	TMDelTList< TMDelTList<int> > Parallel;		//Ìàòðèöà êîëëèíåàðíîñòè-ïàðàëëåëüíîñòè (äëÿ êàæäîé ïàðû ïëîñêîñòé)
	//çàïîëíåíèå ìàòðèö ïóñòûìè çíà÷åíèÿìè
	...
	//ðàñ÷åò ìàòðèöû íîðìàëåé è ìàòðèöû êîëëèíåàðíîñòè
	for (int i=0;i<p_Normal.Count;i++)
	...
	//ñîçäàåì ãåîìåòðè÷åñêèé èíòåðôåéñ êîíòóðà
	TPolygon Contour;
	...
	//çàïîëíåíèé ñïèñêà îòðåçêîâ îòðåçêàìè èñõîäíîãî êîíòóðà êðûøè

	for (int i=0;i<VertexCount;i++)
		AddCut(&GetVertex(i), &GetVertex((i+1) % VertexCount));

	//èíèöèàëèçèðóåì ñïèñîê ïðå-òðèàíãëîâ
/*	for (int i=0;i<PRE_TRI.Count;i++)
	{
...
*/
	//ïåðåñêàåì íå êîëëèíåàðíûå
	//ïîêà òîëüêî íå êîëëèíåàðíûå è íå ïàðàëëåëüíûå

	for (int i=0;i<p_Normal.Count;i++)
	{
		for (int j=i+1;j<p_Normal.Count;j++)
		{
			//äëÿ êàæäîé ïàðû ïëîñêîñòåé
			//ïåðåáèðàåì îñòàëüíûå ïëîñêîñòè è èùåì òî÷êó ïåðåñå÷åíèÿ òðîéè
			if (Parallel[i][j]>=1)
				continue;

			//ïîèñê òî÷êè ïî äâóì ïëîñêîñòÿì

			TIntVec CUR;
			//äâå òî÷êè - ïðÿìàÿ ïåðåñå÷åíèÿ ïàðû ïëîñêîñòåé
			TIntVec P1 = OrgMatrix[i][j];
			TIntVec P2 = OrgMatrix[i][j] + DirMatrix[i][j];
			//òåïåðü ïîëó÷åííóþ ïðÿìóþ ïåðåñåêàåì ñ îñòàëüíûìè ïëîñêîñòÿìè
			for (int k=j+1;k<p_Normal.Count;k++)
			{
				//ïîíÿòíî çäåñü òîëüêî íåêîëëëèíåàðíûå-íåïàðàëëåëüíûå
				if (Parallel[i][k]>=1 || Parallel[j][k]>=1)
					continue;
				//òèïà íå ïåðåñåêàþòñÿ
				//èùåì òî÷êó ïåðåñå÷åíèÿ
				int RoofByPoint = -1;
...
//çàïîëíåíèå êîëëèíåàðíûõ ãðóïï         
//ñíà÷àëà ïðîèñõîäèò ïðîñòî çàïîëíåíèå
//ãðóïï ïëîñêîñòÿìè, à ïîòîì íà îñíîâå ýòîãî èíèöèàëèçàöèÿ ãðóïï

	TMDelTList<TCollinearGroup> Groups;

	for (int i=0;i<p_Normal.Count;i++)
	{
		//â ýòîò ñïèñîê ñîáèðàåì íîìåðà êîëëèíåàðíûõ ïëîñêîñòåé
		...
		{
			//òî îñòàëîñü âíåñòè ïëîñêîñòè â ñîîòâ. ãðóïïó
			//ïîèùåì ãðóïïó
			int gr = FindGroup(Groups,i,-1);
			//åñëè íå ñóùåñòâóåò òî ñîçäàäèì
			if (gr==-1)
				gr = Groups.Add( new TCollinearGroup() );
			//äîáàâèì ïëîñêîñòü i åñëè íàäî
			if ( FindInList(&Groups[gr].indices,i)==-1 )
				Groups[gr].indices.Add(new int(i));
			//åñëè íàäî äîáàâèì ïëîñêîñòè èç ñïèñêà
...
//ïåðåñåêàåì êîëëèíåàðíûå
//â ñîîòâåòñòâèè ñ ïðåäïîëîæåíèåì ëèíèåé ïåðåñå÷åíèÿ
//êîëëèíåàðíûõ ïëîñêîñòåé ÿâëÿåòñÿ çàäàííû íàìè ãðàíèöà
//ïåðåñåêàåì òàêèå ãðàíèöû ñ ñîòàëüíûìè ïëîñêîñòÿìè
	for (int i=0;i<Groups.Count;i++)
		for (int j=0;j<Groups[i].Edges.Count;j++)
			for (int k=0;k<p_Normal.Count;k++)
			{
				if (Groups[i].indices[j] ==k || Groups[i].indices[j+1] == k)
					continue;
				//òåì íå ìåíåå òðåòüÿ ïëîñêîñòü äîëæíà áûòü äàæå íåïàðàëëåëüíà
...
//òåïåðü ñòðîãîå ðàñêèäûâàíèå âñåõ òî÷åê ïåðåñå÷åíèÿ
//ïî îáëàñòÿì ðàçäåëåííûì ãðàíèöàìè êîëëèíåàðíûõ ïëîñêîñòåé
	for (int i=0;i<PntPlane.Count;i++)
	{
		TMDelTList<int> groupindices;
		//áåðåì òî÷êó è ïðîáåãàÿ ïî åå ïëîñêîñòÿì
		//ñìîòðèì íåò ëè ïëîñêîñòè èç êàêîé íèáóäü ãðóïïû êîëëèíåàðíîñòè
		...
		//åñëè íàøëè íåêîòîðûå ãðóïïû êîëëèíåàðíîñòè
		//òî çàïóñêàåì ôóíêöèþ âíåäðåíèÿ òî÷êè â ãðóïïó êîëëèíàðíîñòè
		for(int gr = 0;gr<groupindices.Count;gr++)
			Groups[groupindices[gr]].InsertPoint(Roof.PntPlane[i],&Roof.GetVertex(i),PEPS);
	}

//òåïåðü êàæäîé òðîéêå ïëîñêîñòåé
//ïîñòàâèì â ñîîòâåòñòâèå òî÷êó èõ ïåðåñå÷åíèÿ
//ðàçóìååòñÿ â îäíîé òî÷êå ìîãóò ïåðåñåêàòüñÿ òðè è áîëåå ïëîñêîñòåé
//íî òî÷êà ìîæå áûòü îáðàçîâàíà êàê ìèíèìóì òðåìÿ ïëîñêîñòÿìè

//çàïîëíÿåì PntCube ïóñòûìè çíà÷åíèÿìè
	for (int i=0;i<p_Normal.Count;i++)
	{
...
//ðàñêèäûâàåì ïî ïîëèãîíàì îòðåçêè
//èñõîäíîãî êîíòóðà
...
{                                   
	...
	//â ñëó÷àå íåîæèäàííîãî çàâåðøåíèÿ àëãîðèòìà
	if (minindex==-1)
	{
...
{
	//ïðîõîäèì ïî ñïèñêó òî÷åê è ïðîâåðÿåì
	//ïðèíàäëåæàò ëè åå ñïèñêó ïëîñêîñòåé çàäàííûå ïëîñêîñòè
...
{
		//ñîðòèðóåì èíäåêñû ïëîñêîñòåé ïî
		//ñîîòâ. òî÷êàì êîíòóðà
		TMDelTList<TIntVec> indicesVecs;
		for (int i=0;i<indices.Count;i++)
...
/*
//äîáàâëÿåì â ÊÓÁ òî÷åê î÷åðåíóþ òî÷êó
//ôîðìèðóÿ çàâèñèìîñòü òðè ïëîñêîñòè - ïåðåñå÷åíèÿ
// i < j < k 

//ðàññ÷èòàòü êðèòåðèé äëÿ ïñåâäîòðåóãîëüíèêà
*/
...
/*
//ôóíêöèÿ ðåàëèçóåò âûáðàííûé ïñåâäîòðåóãîëüíèê
//ïî ñîîòâåòñòâóþùèì ñêàòàì
//ñîîòâåòñòâóþùèì â ñìûñëå ñîîòâåòñòâóþùèé ïñåâäîòðåóãîëüíèêó,
//à òàêæå ëåâûé è ïðàâûé îò íåãî
*/
void TRoof::DivideTriangle( TMDelTList<TPreTriangle>& PRE_TRI,int index,TMDelTList<TMTList <TGCut> > &RP )
{
	//ðàáîòàåì ñ ëåâîé ñòîðîíîé ïñåâäîòðåóãîëüíèêà
	...
	{
		//îòðåçîê åùå íå âñòðå÷àëñÿ çíà÷èò ñîçäàåì åãî
	...
	//ðàáîòàåì ñ ïðàâîé ñòîðîíîé ïñåâäîòðåóãîëüíèêà
	...
	{
		//ðàáîòàåì ñ âåðõíåé ñòîðîíîé ïñåâäîòðåóãîëüíèêà
...
/*
//èñïîëüçóåòñÿ íà çàêëþ÷èòåëüíîì ýòàïå ðàáîòû àëãîðèòìà
//êîãäà ñïèñîê îòðåçêîâ ïðèíàäëåæàùèõ ñêàòó
//óïîðÿäî÷èâàåòñÿ â êîíòóð

//ëåâàÿ ñòîðîíà äëèííåå ïðàâîé ñòîðîíû ëåâîãî òðåóãîëüíèêà
*/
...
//ïðàâàÿ ñòîðîíà äëèííåå ëåâîé ñòîðîíû ïðàâîãî òðåóãîëüíèêà
...
{
		//îïðåäåëåíèÿ îäèíàêîâûõ òðåóãîëüíèêîâ ñëåâà è ñïðàâà îò âûáðàííîãî
		//òàêèõ êàêèå èìåþò îäíó òî÷êó ïåðåñå÷åíèÿ
		...
		//ðàñêèäûâàíèå îòðåçêîâ èç ýòèõ òðåóãîëüíèêîâ
		//ïî ñîîòâåòñòâóþùèì ñêàòàì
/*		bool exit = false;
		for (int j=0;j<SameTriangles.Count;j++)
		{
			...
			if ( !Convex.CycVal(index-1) && PRE_TRI[index].CrossPointRight != PRE_TRI[index].CrossPointLeft)
			{
				...
				if (point!=PRE_TRI[index].CrossPointLeft)
				{
					...
					if (cut)
					{
						// äîáàâëÿåì îòðåçîê ñîîòâåòñòâóþùåìó ñêàòó
						...
						// äîáàâëÿåì îòðåçîê ëåâîìó ñêàòó
			...
			if ( !Convex[index] && PRE_TRI[index].CrossPointRight!=PRE_TRI[index].CrossPointLeft)
			{
				...
				if (point!=PRE_TRI[index].CrossPointRight)
				{
					...
					if (cut)
					{
						...
						// äîáàâëÿåì îòðåçîê ïðàâîìó ñêàòó
						if (RESULT[PRE_TRI.CycVal(index+1).Slope][PRE_TRI.CycVal(index+1).Plane].POLSCUT.IndexOf(cut)==-1 )
							RESULT[PRE_TRI.CycVal(index+1).Slope][PRE_TRI.CycVal(index+1).Plane].POLSCUT.Add(cut);
					}
					PRE_TRI[index].TruePointRight = PRE_TRI[index].CrossPointRight; 
...
*/
		...
		//ïðàâûé òðåóãîëüíèê îò ñàìîãî ïðàâîãî è
		//ëåâûé îò ñàìîãî ëåâîãî ïðèíèìàþò íîâûå òî÷êè
		...
		//òðåóãîëüíèêè èç ñïèñêà îäèíàêîâûõ òðåóãîëüíèêîâ óäàëÿþòñÿ
