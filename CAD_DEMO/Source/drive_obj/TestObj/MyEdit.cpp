...
{
    throw EMyException("Íåâîçìîæíî èñïîëüçîâàíèå <TMyFigure::GetCut>");
}

TRedPoint& TMyFigure::GetPoint(int i)
{
    throw EMyException("Íåâîçìîæíî èñïîëüçîâàíèå <TMyFigure::GetPoint>");
}
...
//ôóíêöèÿ ïðèâÿçêè íà÷àëüíîé òî÷êè
void TRedCut::SetSrc(TRedPoint* Value)
{
    AboutToChange(this);
    //åñëè íà÷. òî÷êà óæå çàäàíà òî
    //óáåðåì ñåáÿ èç åå ñïèñêà îòðåçêîâ
    if (FSrc)
        FSrc->Cuts.Remove(this);

    //çàäàåì íîâóþ íà÷. òî÷êó
    FSrc = Value;

    //åñëè íà÷. òî÷êà óæå çàäàíà òî
    //äîáàâèì ñåáÿ â ñïèñîê åå îòðåçêîâ
    if (FSrc)
        FSrc->Cuts.Add(this);
}
...
{
    AboutToChange(this);
    //åñëè êîí. òî÷êà óæå çàäàíà òî
    //óáåðåì ñåáÿ èç åå ñïèñêà îòðåçêîâ
    if (FDst)
        FDst->Cuts.Remove(this);
    //çàäàåì íîâóþ êîí. òî÷êó
    FDst = Value;
    //åñëè êîí. òî÷êà óæå çàäàíà òî
    //äîáàâèì ñåáÿ â ñïèñîê åå îòðåçêîâ
    if (FDst)
...
/*//íàõîäèì îòðåçîê ïî äâóì òî÷êàì
