...
TGCut *TGeomObject::AddCut(TGPoint *p1, TGPoint *p2)
{
	if (FVertex.IndexOf(p1) == -1)
		throw EMyException ("<TGeomObject::AddCut> ïåðâîé òî÷êè íåò â îòðåçêå!");

	if (FVertex.IndexOf(p2) == -1)
		throw EMyException ("<TGeomObject::AddCut> âòîðîé òî÷êè íåò â îòðåçêå!");

	if (p2 == p1)
		throw EMyException ("<TGeomObject::AddCut> íåâîçìîæíî äîáàâèòü îòðåçîê ñ ðàâíûìè êîíöàìè!");
...
TGPolygon *TGeomObject::CopyPolygonToObject(const TGPolygon &poly )
{
	...
	//Äîáàâëÿåì òî÷êè ïîëèãîíà òàê, ÷òîáû íå ïîëó÷èëîñü îòðåçêîâ èç äâóõ îäèíàêîâûõ òî÷åê(Â íîâîì îáüåêòå ðàçíûå òî÷êè ìîãóò ñòàòü îäíîé)
	...
	for (int i=0;i<poly.FHoles.Count;i++)
	{
		...
		// Äîáàâëÿåì òî÷êè ïîëèãîíà òàê, ÷òîáû íå ïîëó÷èëîñü îòðåçêîâ èç äâóõ îäèíàêîâûõ òî÷åê
		// (Â íîâîì îáüåêòå ðàçíûå òî÷êè ìîãóò ñòàòü îäíîé)
...
TGPolygon *TGeomObject::AddPolygon(TMTList<TGCut> &cuts, const TIntVec &nrm)
{
	if (cuts.Count<3)
		return NULL;
//		throw EMyException ("<TGeomObject::AddPolygon> íåäîñòàòî÷íî äàííûõ!");
	...
	if (!p->Closed)
		throw EMyException ("<TGeomObject::AddPolygon> äîáàâëåí íåçàìêíóòûé ïîëèãîí!");
...
{
	//ïðîñòî ïåðåáèðàåì ñïèñîê îòðåçêîâ îäíîé èç òî÷åê
	if (P1==NULL)
		return NULL;
	if (P2==NULL)
		return NULL;
...
bool TGeomObject::Contourize2d(const TMTList<TGCut> &Cuts, TMTList<TGCut> &contour, TGPoint *begin)
{
	...
	if (begin == NULL) 
		begin = GetLowestPoint2d(cuts);					//Íàõîäèì ñàìóþ íèæíþþ òî÷êó
	TGCut	*cut = begin->GetTurnLeftCut(true);		//Íàõîäèì îòðåçîê, ñàìûé ïîâîðà÷èâàþùèé íàïðàâî
//	TGCut	*begCut = cut;
	...
	do
	{
		...
		if (i>Cuts.Count*100)
			if (MessageBox(NULL,"Ïîëó÷åíî â 100 ðàç áîëüøå îòðåçêîâ, ÷åì âõîäíûå äàííûå. Âîçìîæíî, ïðîèçîøëî çàöèêëèâàíèå. Ïðåðâàòü îïåðàöèþ?", "TGeomObject::Polygonze2d", MB_YESNO | MB_ICONQUESTION | MB_SYSTEMMODAL) == ID_YES)
				break;
			else
				i = 0;	
...
//DeleteBranches(cuts);
		...
		if (cut->FFlags.Contour != gfTRUE)
		{
			...
			for (int i=0;i<contour.Count;i++)
				for (int j=i+1;j<contour.Count;j++)
					if (contour.Items[i] == contour.Items[j])
                		throw EMyException("drPolygonize: âåòêè ïîñëå Contourize");
						//cuts.Remove(contour.Items[i]);
...
void TGeomObject::RestorePoints()
{
	if (!PointsCache.Count)
		throw EMyException("<TGeomObject::RestorePoints> : Â ñòåêå íè÷åãî íåò!");
	...
	for (int i=0;i<p->FCuts.Count;i++)
		for (int j=0;j<p->FCuts[i].FPlane.Count;j++)
			if (p->FCuts[i].FPlane.Items[j] && p->FCuts[i].FPlane.Items[j] != p)
			{
				...
				if (!pCut || !oCut)
					throw EMyException("<SynchronizePolygonsWisesRecursive> : ÷òî-òî íå òàê!");
...
bool TGeomObject::SolidCheck()
{
	...
	for (int i=0;i<FCuts.Count;i++)
	{
		if (FCuts[i].Src == NULL || FCuts[i].Dst == NULL)
			ErrorMsg("<TGeomObject::SolidCheck> : îáíàðóæåí îòðåçîê, íå èìåþùèé îäíîãî èç êîíöîâ");
		if (!FCuts[i].Closed)
		{
	...
	{
		if (!FPolygons[i].Closed)
			ErrorMsg("<TGeomObject::SolidCheck> : îáíàðóæåí íåçàìêíóòûé ïîëèãîí");
		if (FPolygons[i].FCuts.Count<3)
			ErrorMsg("<TGeomObject::SolidCheck> : â ïîëèãîíå ìåíüøå 3-õ îòðåçêîâ");
		if (FPolygons[i].FCuts.IndexOf(NULL) != -1)	
			ErrorMsg("<TGeomObject::SolidCheck> : ïîëèãîí èìååò ïóñòîé îòðåçîê");
			
		for (int k=0;k<FPolygons[i].Count;k++)
			for (int l=k+1;l<FPolygons[i].Count;l++)
				if (&FPolygons[i].GetPoint(k) == &FPolygons[i].GetPoint(l))
					ErrorMsg("<TGeomObject::SolidCheck> : ïîëèãîí ñîäåðæèò îäèíàêîâûå âåðøèíû!");

		for (int j=0;j<FPolygons[i].FHoles.Count;j++)
		{
			if (!FPolygons[i].FHoles[j].Closed)
				ErrorMsg("<TGeomObject::SolidCheck> : îáíàðóæåí íåçàìêíóòûé ïîëèãîí-äûðêà");
			if (FPolygons[i].FHoles[j].FCuts.Count<3)
				ErrorMsg("<TGeomObject::SolidCheck> : â ïîëèãîíå-äûðêå ìåíüøå 3-õ îòðåçêîâ");
			if (FPolygons[i].FHoles[j].FCuts.IndexOf(NULL) != -1)	
				ErrorMsg("<TGeomObject::SolidCheck> : ïîëèãîí-äûðêà èìååò ïóñòîé îòðåçîê");
		}	
	}
	...
	for (int i=0;i<FPolygons.Count;i++)
		for (int j=0;j<FPolygons[i].FHoles.Count;j++)
			if (FPolygons[i].FHoles[j].FParent != FPolygons.Items[i])
				ErrorMsg("<TGeomObject::SolidCheck> : äûðêà â ïîëèãîíå íå çíàåò ïðî íåãî!");

	for (int i=0;i<FPolygons.Count;i++)
		if (FPolygons[i].FParent != NULL)
			ErrorMsg("<TGeomObject::SolidCheck> : Ó ïîëèãîíà åñòü ðîäèòåëü!");
				

	for (int i=0;i<FCuts.Count;i++)
		if (FCuts[i].FPlane.Count>2)
			WarningMsg("Îòðåçîê ïðèíàäëåæèò áîëåå, ÷åì äâóì ïîëèãîíàì!");
