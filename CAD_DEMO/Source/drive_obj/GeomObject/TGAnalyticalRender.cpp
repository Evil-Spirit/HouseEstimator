...
/*    for ( int i = 0; i < FCuts.Count; i++ )
        for ( int j = i+1; j < FCuts.Count; j++ )
            if ( FCuts[i].Src == FCuts[j].Src && FCuts[i].Dst == FCuts[j].Dst ||
                ...
                throw EMyException("ProcessCrossCuts: Íàëîæåíèå îòðåçêîâ");*/
...
//                if ( TmpPoly.IndexOf(FPolygons.Items[j]) == -1 )
  //                  TmpPoly.Add(FPolygons.Items[j]);
    //            else
      //              ErrorMsg("AnalRen: Ïîâòîðíûé ïîëèãîí!");
...
//                      if ( obj.FPolygons.Count > FCuts[i].FPlane[ii].Tag && FCuts[i].FPlane[ii].Tag >= 0 )
            ...
            if ( ii == FCuts[i].FPlane.Count )
                if ( Inds.IndexOf(FCuts.Items[i]) == -1 )//íà âñÿêèé ïîæàðíûé, óáðàòü ïðè îïòèìèçàöèè
                {
    ...
    for ( j = 0; j < cuts.Count; j++ )
    {
        ...
        if ( PIPQ != PIPC )
            if ( PIPQ == 0 )
                ...
                throw EMyException("<TGeomObject::ExistsPolygonInPolygonUseHoles2>: Íå îòñëåæåííàÿ ñèòóàöèÿ!");
    }
...
/*                    TMTList<TGCut> TmpCuts;
                    ...
                    ErrorMsg("<TGeomObject::IntersectPolygonsUseHoles>: Äûðêà â äûðêå!");
                }
        }
}
...
TIntVec TGeomObject::PointInPolygon(TGPolygon* p)
{
    ...
    if ( i == p->Count )
    {
        ErrorMsg("< TGeomObject::PointInPolygon>: Òî÷êè ïîëèãîíà íà îäíîé ëèíèè");
        for ( int i = 0; i < GHP->Count; i++ )
...
//        throw EMyException ("< TGeomObject::PointInPolygon>: Òî÷êè ïîëèãîíà íà îäíîé ëèíèè");
...
TIntVec TGeomObject::GetPointInPolygonUseHoles(TGPolygon* p)
{
    ...
    if ( i == p->Count )
    {
        ErrorMsg("< TGeomObject::PointInPolygonUseHoles>: Îøèáêà âûáîðà òî÷åê ïðîâåðêè");
        for ( int i = 0; i < GHP->Count; i++ )
            GHP->GetPoint(i).Point = TransformFrom(GHP->GetPoint(i).Point, ang);
