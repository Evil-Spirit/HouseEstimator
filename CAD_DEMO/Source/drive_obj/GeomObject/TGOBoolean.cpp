...
void TGeomObject::TransformToPlane(const TIntVec& n, TMTList<TMyObject> &what, MBTi &AngX, MBTi &AngY, const TIntVec &org)
{
	...
	if (what.Items[i])
	{
		...
		{
			throw EMyException(what[i].DynamicType->Name + " <TGeomObject::TransformToPlane> : íåâåðíûé òèï!");
		}	
	}	  
...
void TGeomObject::TransformBack(TMTList<TMyObject> &what, const MBTi &AngX, const MBTi &AngY, const TIntVec &org)
{
	...
	for (int i=0;i<what.Count;i++)
	{
		...
		{
			throw EMyException("<TGeomObject::TransformBack> : íåâåðíûé òèï!");
		}	
	}	
		...
		if (c->FPlane.Items[i])
		{
			...
			if (c->FPlane[i].FCuts.CycVal(index-1).CanConnect(*cn))
			{
				...
				if (c->FPlane[i].FCuts.IndexOf(cn1)!=-1 || c->FPlane[i].FCuts.IndexOf(cn)!=-1)
					throw EMyException("<TGeomObject::SplitCut> - íàëîæåíèå îòðåçêîâ!");
#endif				
...
{
	if (!p)
		throw EMyException("TGeomObject::TransfixPolygon : NULL - ïîëèãîí");
	cps.Clear();
	TMTList <TGCut> cuts;
	//Çàïîìèíàåì îòðåçêè ïîëèãîíà
	cuts.Assign(&p->FCuts);

	// Ðàçðåçàåì îòðåçêè ïîëèãîíà
...
//ñîðòèðóåì òî÷êè ðàññå÷åíèÿ
	for (int i=0;i<cps.Count;i++)
		for (int j=i;j<cps.Count;j++)
			if (cps[i].Point.x>cps[j].Point.x)
				cps.Exchange(i,j);

//Óäàëÿåì ïîâòîðíûå òî÷êè ðàññå÷åíèÿ				
	for (int i=0;i<cps.Count;i++)
...
/*
	// Ïðîâåðÿåì êàñàíèå îòðåçêîâ ðàçáèåíèÿ ñ äûðêàìè
	for (int j=0;j<p->FHoles.Count;j++)
	{
		...
		// Åñëè åñòü êàñàíèå - äîáàâèòü îòðåçêè äûðêè â êó÷ó, ïðè ýòîì óäàëèâ åå èç èç ðàçðÿäà äûðîê
		if (linkH)
		{
			for (int k=0;k<p->FHoles[j].FCuts.Count;k++)
				Heap.Add(p->FHoles[j].FCuts.Items[k]);
			// Òîëüêî åñëè äûðêà ïåðåñòàåò áûòü äûðêîé ìû åå óäàëÿåì	
			if (linkC)
				p->FHoles.Delete(j--);
		}		
	}			
...
*/				
	...
	// Óäàëÿåì ðàçáèâàåìûé ïîëèãîí
	FPolygons.Remove(p);
	Result.Clear();

	TMDelTList< TMTList<TGCut> > Poly;		//Ïîëèãîíû
...
void TGeomObject::SplitPolygonByPlane(TGPolygon *p, const TIntVec& n, const TIntVec& o, TMTList<TGPolygon> &Result)
{
	...
	//Íàõîäèì ëèíèþ ïåðåñå÷åíèÿ	
	if (!PlanePlaneCross(n, o, p->Normal, p->Origin, dir, org, PEPS)) 
		return;

	//Ïîâîðà÷èâàåì îáúåêò â ïëîñêîñòü ïîëèãîíà
...
{
	cps.Clear();
	// Ðàçðåçàåì îòðåçêè ïëîñêîñòüþ äðóãîãî ïîëèãîíà
	for (int i=0;i<p->FCuts.Count;i++)
	{
		TIntVec it;
	...
	//Ðàçðåçàåì äûðêè ïîëèãîíà ïëîñêîñòüþ äðóãîãî ïîëèãîíà
	for (int i=0;i<p->FHoles.Count;i++)
	{
		// Ðàçðåçàåì îòðåçêè äûðêè
		for (int j=0;j<p->FHoles[i].FCuts.Count;j++)
...
/*
	...
	TMTList <TGCut> cuts;			// áóôôåð îòðåçêîâ ïîëèãîíîâ
	TMDelTList <TGPolygon> holes;	// äûðêè....
	TMTList < TGPoint > cps;

	// çàïîìèíàåì îòðåçêè ïåðâîãî ïîëèãîíà
	...
	// çàïîìèíàåì îòðåçêè âîòîðîãî ïîëèãîíà  
	cuts.Assign(&p2->FCuts);

	// ïåðåñåêàåì îòðåçêè ñ ïëîñêîñòüþ äðóãîãî ïîëèãîíà
	for (int i=0;i<cuts.Count;i++)
	{
		TGCut *r1, *r2;
		TGPoint *cp = SliceCutByPlane2d(cuts.Items[i], r1, r2);
	...
	// Èíäåêñíûé áóôôåð îáùèõ îòðåçêîâ
	...
	// Íàõîäèì îáùèå îòðåçêè
	for (int i=0;i<cps.Count-1;i++)
	{
		// Ïðèíàäëåæíîñòü îòðåçêîâ ïîëèãîíàì
		// Ýïñèëîí ïðèíàäëåæíîñòè äîëæåí áûòü â íåñêîëüêî ðàç ìåíüøå ýïñèëîíà òî÷åê
		...
		// Åñëè îòðåçîê ïðèíàäëåæèò îáîèì ïîëèãîíàì, äîáàâëÿåì îáùèé îòðåçîê
...
//		r2.Last()->FFlags.Boundary = (i<borCount2) ? gfTRUE : gfFALSE; 
	...
	// Âîçâðàùàåì ïîëèãîíû ñ ïëîñêîñòè
...
////////////////
	...
	// Ñîîòâåñòñòâèå ïîëèãîíîâ è îòðåçêîâ ïåðåñå÷åíèÿ
	...
	// Îáüåêò, â êîòîðîì ïðîèçâîäÿòñÿ ïåðåñå÷åíèÿ ïîëèãîíîâ
	TGeomObject temp;
	// Îáüåêò - õðàíèëèùå îòðåçêîâ øâà ïåðåñå÷åíèÿ îáüåêòîâ
...
//	result.append(obj);
	...
	// Èíèöèàëèçèðóåì ñïèñêè ñîîòâåòñòâèÿ
	for (int i=0;i<FPolygons.Count;i++)
	{
...
///////////////////////////	
	...
	//	Îòêëþ÷åíèå ïðèâÿçîê òî÷åê â temp
	//	Òåì íå ìåíåå, ïðèâÿçêà øâà ê îòðåçêó áóäåò îñóùåñòâëÿòüñÿ âíóòðè ôóíêöèè SliceCutByPlane2d
	temp.SnapPoints = false;

	//	Âêëþ÷åíèå îïöèè óëàâëèâàíèÿ øâà ïî øâó ïî êðèòåðèþ íàèáîëåå áëèæíèõ òî÷åê
...
//			temp.Clear();
			...
			// êîïðèðîâàíèå äâóõ ïåðåñåêàåìûõ ïîëèãîíîâ â temp
			TGPolygon *p1 = pl1[i].Target;
			TGPolygon *p2 = pl2[j].Target;

			// Åñëè ïðîèçîøëî ïåðåñå÷åíèå - äîáàâèòü îòðåçêè øâà
...
/*			{
				...
				{
					// Êîïèðóåì îòðåçîê â Intersect
					TGCut *ct = intersection.CopyCutToObject(split1[k]);
					
					// Åñëè åãî íåò â ñïèñêå ïðèíàäëåæíîñòåé è îí íå ñâåðíóëñÿ â òî÷êó, äîáàâëÿåì åãî
