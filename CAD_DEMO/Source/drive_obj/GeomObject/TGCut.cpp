...
//SameDIrection ðàáîòàåò ïðè óñëîâèè, ÷òî CanConnect() != NULL
...
TGPoint *TGCut::GetAnotherPoint(const TGCut &c) const
{
	...
	if (!c.ConsistsPoint(Dst)) 
		return Dst;
	throw EMyException("<TGCut::GetNonCommonPointWith>: íåò ñâÿçè ñ äàííûì îòðåçêîì");		
}
...
TGPoint *TGCut::GetAnotherPoint(const TGPoint* Point) const
{
	if (Src == Point)
		...
		return Src;
	throw EMyException("<TGCut::GetAnotherPoint>: íåò òàêîé òî÷êè");		
}
/*
TGPolygon *TGCut::GetAnotherPolygon(TGPolygon* Plane)
{
	if (Left == Plane)
		...
		return Left;
	throw EMyException("<TGCut::GetAnotherPolygon>: íåò òàêîé ïëîñêîñòè");		
}
*/ 
