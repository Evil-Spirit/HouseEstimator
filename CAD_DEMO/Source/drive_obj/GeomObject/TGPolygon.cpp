...
TGPolygon::TGPolygon(TMTList <TGCut> &cuts)
{
	...
	for (int i=0;i<cuts.Count;i++)
		if (AddCut(cuts.Items[i]) == grERROR)
			ErrorMsg("<TGPolygon::TGPolygon> íåâîçìîæíî äîáàâèòü îòðåçîê!");
}
...
bool TGPolygon::Equal(const TGPolygon& other) const
{
	if (FCuts.Count != other.FCuts.Count)
		return false;
	for (int i=0;i<FCuts.Count;i++)
		if (FCuts.Items[i] != other.FCuts.Items[i])
			return false;
	return true;
}
...
/*	if (Closed)
	{
		throw EMyException("<TGPolygon::AddCut> äîáàâëåíèå îòðåçêà â çàìêíóòûé ïîëèãîí");
		return grERROR;
	}	
*/
	if (!Cut)
		throw EMyException("<TGPolygon::AddCut> íåâîçìîæíî äîáàâèòü NULL");
		
	if (FCuts.IndexOf(Cut) != -1)
		throw EMyException("<TGPolygon::AddCut> íåâîçìîæíî äîáàâèòü ñóùåñòâóþùèé îòðåçîê!");
//		return grERROR;
    ...
    // Ñíà÷àëà íóæíî Ïðîâåðèòü êîíåö - ÷òîáû íå ïîëó÷èòü èíâåðñèè îòðåçêîâ.
	if ( canConnectToEnd)
	{
		FCuts.Add(Cut);
		Cut->AddPlane(this);			
...
//			if (a<PEPS || b<PEPS)
//				throw EMyException("<TGPolygon::FindBasisPoints> - ïëîõîé ïîëèãîí!");
...
bool TGPolygon::CalculatePlane()
{
	...
	if (!FindBasisPoints(basis))
		throw EMyException("<TGPolygon::CalculatePlane> Íåâîçìîæíî ðàññ÷èòàòü íîðìàëü!");
...
void TGPolygon::RemoveCut(TGCut *cut)
{
	FClosed = false;
	if ( cut->RemovePlane(this) != -1 )
		FCuts.Remove(cut);
	else
		throw EMyException("TGPolygon::RemoveCut : îòðåçîê â ïîëèãîíå íå çíàåò î íåì.");
}
...
void TGPolygon::DeleteCut(int index)
{
	...
	if ( FCuts.Items[index]->RemovePlane( this ) != -1 )
		...
		throw EMyException("TGPolygon::DeleteCut : îòðåçîê â ïîëèãîíå íå çíàåò î íåì.");
...
/*
void PointTo2D(TIntVec& Point,const TIntVec& ZAngle,const TIntVec& XAngle,const TIntVec& RCenter)
{
	Point = RotateAround(Point,ZAngle,RCenter);
	Point = RotateAround(Point,XAngle,RCenter);
}

void PointTo3D(TIntVec& Point,const TIntVec& ZAngle,const TIntVec& XAngle,const TIntVec& RCenter)
{
	Point = RotateAround(Point,XAngle*(-1),RCenter);
	Point = RotateAround(Point,ZAngle*(-1),RCenter);
}
*/
...
TGPolygon *TGPolygon::AddHole(TMTList<TGCut> &cuts)
{
	...
	if (!p->Closed)
		throw EMyException ("<TGeomObject::AddPolygon> äîáàâëåí íåçàìêíóòûé ïîëèãîí-äûðêà!");
...
{
	if (FFlags.NeedCalc == gfTRUE)
	{
		CalculatePlane();
		FFlags.NeedCalc = gfFALSE;
	}
	return FNormal;	
}
...
/*	for (int k=0;k<Count;k++)
		for (int l=k+1;l<Count;l++)
			if (&GetPoint(k) == &GetPoint(l))
				throw EMyException("<TGeomObject::SolidCheck> : ïîëèãîí ñîäåðæèò îäèíàêîâûå âåðøèíû!");
*/
