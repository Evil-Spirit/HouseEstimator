...
#include "Poligon.h"
#include "TestTriangulation.h"

//---------------------------------------------------------------------------

TDelone2* pDeloune2 = NULL;

TDelone2& GetDelone2()
{
    return *GetDeloune2();
}
TDelone2* GetDeloune2()
{
    if (pDeloune2 == NULL)
        pDeloune2 = new TDelone2();
    return pDeloune2;
}


bool TDelone2::SimpleIntersection(TDSide& a,TDSide& b)
{
...
bool TDelone2::CheckBldCrossing(int i, int i1, int i2)
{
...
//-= ïåðåñåêàþòñÿ ëè ïîñòðîåííûå ñ èñõîäíûìè ðåáðàìè ?
bool TDelone2::CheckSimpleCrossing(int i, int i1, int i2)
{
    TDSide r1(pAll.Items[i],pAll.Items[i1]);
    TDSide r2(pAll.Items[i],pAll.Items[i2]);
...
void TDelone2::FindBaseRibs(int& i1,int& i2)
{
    //-= 1-ÿ áàçîâàÿ òî÷êà
    for (int i=0; i<pAll.Count; i++)  //åùå óñëîâèå ïî x
...
bool TDelone2::FindDelone2Brother(int i1,int i2)
{
                TDSide *nSide = new TDSide(pAll.Items[i1], pAll.Items[i2]);
                for (int i=0; i<rBld.Count; i++)
                    if (*nSide == rBld[i] || SimpleIntersection(*nSide, rBld[i])) //-= Áûëî ëè óæå ðåáðî ?
                    {
                        delete nSide;
                        return false;
                    }
    ...
    if (_max != 1)
    {
        bool f = FindDelone2Brother(i1, iN); //-= çàïóñòèì 2 íîâûå áàçîâûå ëèíèè
        bool s = FindDelone2Brother(iN, i2);
        bool not_cross = !CheckBldCrossing(iN, i1, i2);
        if ( not_cross && (f || s) )                //-= íàøëè íîâûé òåðóãîëüíèê
        {
...
TDSide* TDelone2::FindRibByPoints(TDVec* P0,TDVec* P1)
{
    TDSide Side(P0,P1);
    for (int i=0;i<rBld.Count;i++)
        if ( rBld[i] == Side )
...
bool TDelone2::FindDelone2Brother_Ribs(int i1,int i2)
{
                TDSide *nSide = new TDSide(pAll.Items[i1], pAll.Items[i2]);
                for (int i=0; i<rBld.Count; i++)
                    if (*nSide == rBld[i] || SimpleIntersection(*nSide, rBld[i])) //-= Áûëî ëè óæå ðåáðî ?
    ...
    if (_max != 1)
    {
        bool f = FindDelone2Brother_Ribs(i1, iN); //-= çàïóñòèì 2 íîâûå áàçîâûå ëèíèè
        bool s = FindDelone2Brother_Ribs(iN, i2);
        bool not_cross = !CheckBldCrossing(iN, i1, i2);
        if ( not_cross && (f || s) )                //-= íàøëè íîâûé òåðóãîëüíèê
        {
...
void TDelone2::MergePolygon(TPolygon* P)
{
...
void TDelone2::MergeHPolygon(THoledPolygon* HP)
{
    MergePolygon(HP);
...
void TDelone2::DeleteClosePoints()
{
    for (int i=0;i<pAll.Count;i++)
...
int TDelone2::FindClosePoint(const TDVec& Vec,MBTi USEEPS)
{
    for (int i=0;i<pAll.Count;i++)
...
void TDelone2::ProcessCrossList()
{
    bool OrgnOrgn = false;
    bool OrgnDest;
    bool DestOrgn;
 ...
 /*           if ( int(iC_Orgn)+int(iC_Dest)+int(jC_Orgn)+int(jC_Dest)>=2 )
            ...
            //ïðîñòî ñîäåðæèò òî÷êó
            //íå ðàññìîòðåí ñëó÷àé êîãäà
...
void TDelone2::TriangulateByPoints()
{
    int i1 = 0;
    int i2;
    FindBaseRibs(i1,i2);
    FindDelone2Brother(i1,i2);
}

void TDelone2::TriangulateByRibs()
{
    int i1 = 0;
    int i2;
    FindBaseRibs(i1,i2);
    FindDelone2Brother_Ribs(i1,i2);
}

TMTList<TIntVec>* TDelone2::Triangulate(THoledPolygon* HP)
{
    Clear();
    MergeHPolygon(HP);
    DeleteClosePoints();
...
TMTList<TIntVec>* TDelone2::Triangulate2(TMTList<THoledPolygon>* HPL,TMyPolOperation pOP)
{
    Clear();
...
TMTList<TIntVec>* TDelone2::Triangulate3(THoledPolygon* HP,TMTList<TIntVec>* LineEdges)
{
    Clear();
    MergeHPolygon(HP);
...
TMTList<TIntVec>* TDelone2::OPERATION(TPolygon* P0,TPolygon* P1,int oper)
{
    Clear();
    Started = true;
    MergePolygon(P0);
    ...
    int nol_index = Contour.IndexOf(P0->Vertex->Items[0]);
...
TMTList<TIntVec>* TDelone2::GetContour(TPolygon* SelfCrossed)
{
    return NULL;
};
