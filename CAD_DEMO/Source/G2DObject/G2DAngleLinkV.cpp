...
{
    if (!GPoint)
        throw EMyException("<TAngleLink::IsBestToUpdate>: Îøèáêà");
...
bool TAngleLink::UpdatePointRule(TG2DPoint* GPoint)
{
    ...
    if ( GPoint == Client0->CanConnect(*Client1) &&
    	((TG2DPoint*)Client0->GetAnotherPoint(GPoint))->Rules.Count == 2 &&
        ((TG2DPoint*)Client1->GetAnotherPoint(GPoint))->Rules.Count == 2)
    {
        MBTi UseAngle = Angle;
    ...
    {
        AnsiString Str = "<TAngleLink::UpdateClient>: òî÷êà íå ïðèíàäëåæèò ñâÿçêå";
...
bool TAngleLink::CanUpdateList(TMTList<TG2DPoint>& PointList)
{
	bool Result = false;
    bool Client0Fixed = ((TG2DCut*)Client0)->Rules.Count>0;
	bool Client0SrcFixed = ((TG2DPoint*)Client0->Src)->Rules.Count==2;
	bool Client0DstFixed = ((TG2DPoint*)Client0->Dst)->Rules.Count==2;

    bool Client1Fixed = ((TG2DCut*)Client1)->Rules.Count>0;
	bool Client1SrcFixed = ((TG2DPoint*)Client1->Src)->Rules.Count==2;
	bool Client1DstFixed = ((TG2DPoint*)Client1->Dst)->Rules.Count==2;

	if (Client0Fixed && (Client0SrcFixed+Client0DstFixed)==1)
    {
     	PointList.Add( (Client0SrcFixed)? ((TG2DPoint*)Client0->Dst) : ((TG2DPoint*)Client0->Src) );
        Result = true;
    }

	if (Client1Fixed && (Client1SrcFixed+Client1DstFixed)==1)
    {
     	PointList.Add( (Client1SrcFixed)? ((TG2DPoint*)Client1->Dst) : ((TG2DPoint*)Client1->Src) );
        Result = true;
    }

    TG2DPoint* Connection = (TG2DPoint*)Client1->CanConnect(*Client0);
    if ( Connection )
    	if ( ((TG2DPoint*)Client1->GetAnotherPoint(Connection))->Rules.Count==2 &&
             ((TG2DPoint*)Client0->GetAnotherPoint(Connection))->Rules.Count==2 )
        	if ( PointList.IndexOf(Connection) == -1)
            {
	     		PointList.Add( Connection );
				Result = true;
            }
	return Result;
}
