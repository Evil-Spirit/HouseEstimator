...
#define G2DGeomVH
//---------------------------------------------------------------------------
const MBTf MBTf_EPS = 0.0000000001; //10^(-10)  äëÿ ðàñ÷åòîâ ñ ÷èñòî MBTf
const MBTf THRESHOLD_EPS = 0.0000001; //10^(-7) òî÷íîñòü ãðàíè÷íàÿ äëÿ MBTi è MBTf
const MBTi MBTi_EXACT_EPS = 0.00001; //10^(-5) íàèáîëåå íèçêèé EPS äëÿ MBTi

const MBTf MBTf_MIN = -MaxSingle;
const MBTf MBTf_MAX = MaxSingle;
const int MaxDivisionCount = 360; // ìàêñ ÷èñëî ðàçáèåíèé ïðè ïîèñêå îáëàñòè îïðåäåëåíèÿ ïîëÿðíîé ôóíêöèè
...
public:
    TMFunction(){};
    ~TMFunction(){};
    virtual TIntVec CalcPolarValue(MBTf arg)const {throw EMyException("<TMFunction::CalcPolarValue>: íåò ðåàëèçàöèè.");};
    virtual MBTf CalcValue(MBTf arg) const {throw EMyException("<TMFunction::CalcPolarValue>: íåò ðåàëèçàöèè.");};
