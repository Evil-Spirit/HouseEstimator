...
{
	//âîçâðàùàåò äëèíó è çíàê íàïðàâëåíèå äëÿ èñïîëüçîâàíèÿ âìåñòå ñ Peppendicular2D
                        ...
                        MBTi USEEPS)
{
    switch ( IsLinesCrossedExactly(A0,A1,B0,B1,CP/*,USEEPS*/) )
    {
        ...
        case lcPARALLEL:
            return false;
        default:
            throw EMyException("<CrossPointNotStrictly>: íåîæèäàííûé ðåçóëüòàò IsLinesCrossed");
    }
}
                    ...
                    TMDelTList<TIntVec>& CPS,bool Strongly,MBTi USEEPS)
{
    if ( P1.Equal(P2,USEEPS) )
        return false;
    //ðàñ÷èòàåì ðàññòîÿíèå îò ïðÿìîé äî öåíòðà îêðóæíîñòè êàê âûñîòó òðåóãîëüíèêà
    //÷åðåç ïëîùàäü
...
{
    //óæå ïðåäïîëàãàåòñÿ ÷òî äèñêðèìèíàíò îòðèöàòåëüíûé
    MBTf x0 = Center.x;
    MBTf x0_2 = x0*x0;
    MBTf x0_4 = x0_2*x0_2;
...
//    MBTf R_4 = R_2*R_2;
    ...
    if (DD<-EXACT_EPSILON)
        throw EMyException("<TPolarCircle::MoveToEdge>: Îøèáêà.");
	DD = sqrtDC(DD);
    TMDelTList<MBTf> Xs;
...
bool TPascalLimacon::CrossingWithLine(const TIntVec& P0,const TIntVec& P1,TMDelTList<TIntVec>& Roots,MBTf USEEPS) const
{
    ...
    if (lc == 0)
    {
        //Ýòîò ñëó÷àé íå ðàññìîòðåí
        throw EMyException("<TPascalLimacon::CrossingWithLine>: Ñëó÷àé ïðè lc==0 íå ðàññìîòðåí.");
    }
    else
    {
...
//ïðè óñëîâèè ÷òî íà ýòîì ïðîìåæóòêå îäèí èíòåðâàë
