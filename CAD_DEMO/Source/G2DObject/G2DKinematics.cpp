...
bool TFlowInfo::FindAngleEdgesAndLinkedGroupsRec(TG2DCut* NotCut,TG2DCut* Cut,int index)
{
    ...
    for (int i=0;i<Cut->LinksCount;i++)
        if ( Cut->GetLink(i).Is( TAngleLink::StaticType ) )
        {
            ...
            {
                ErrorMsg("<FindAngleTreeRec>: Ýñêèç ïåðåîïðåäåëåí.");
                return false;
            }
...
bool TFlowInfo::FindAngleEdges()
{
	...
	//óäàëåíèå ñâÿçíûõ îòðåçêîâ ïîëíîñòüþ íàõîäÿùèõñÿ âíóòðè ñêåëåòîíà, ïîêà íå íàäî
    

	//íàõîæäåíèå íîæåê ñêåëåòîíà
    for (int i1=0;i1<CutGroups.Count;i1++)
	    for (int s1=0;s1<CutGroups[i1].Skeletons.Count;s1++)
        {
			...
			if (Legs.Count>1)
            {
                ...
                if ( Skeletons[ SkeletonIndex ].Legs.Count>0 )
                	throw EMyException("<TFlowInfo::FindAngleEdges>: îøèáêà.");
...
bool TSkeleton::CheckLimacon()
{
	if (LimaconOK)
    	return true;
	if (Legs.Count==0)
    	return true;//íåò íîæåê
//    if ( IndexOfCut( &Legs[0] )==-1 )
//    	return true;//íåò çàôèêñèðîâàííûõ ïî óãëó íîæåê
    ...
    if ( SkeletonLegPoints.Count<2 )
    	return true;//íåò íîæåê ñ çàôèêñèðîâàííûìè òî÷êàìè
	//-------------------------------------------------------
    if ( FCheckedRules.Count > 1 )
    {
//    	throw EMyException("<TSkeleton::CheckLimacon>: ïåðåîïðåäåëåíèå.");
        return true;
    }
	//-------------------------------------------------------
    LimaconOK = true;
    ...
    if ( SkeletonLegPoints.Count<2 )
    	throw EMyException("<TSkeleton::CheckLimacon>: îøèáêà.");
...
bool TSkeleton::Check()
{
    if ( FCheckedRules.Count > 3 )
	  	throw EMyException("<TSkeleton::Check>: ïåðåîïðåäåëåí.");

    if ( FCheckedRules.Count == 3 )
    {
    	if ( FCheckedPoints.Count == 1 )
        {
        	//ñèòóàöèÿ êîãäà çàôèêñèðîâàí îòðåçîê
            //íî íè ñ ÷åì íå ñâÿçàí
            throw EMyException("<TSkeleton::Check>: Îøèáêà.");
        }
...
bool TFlowInfo::RegisterPointFreedomDegree(TG2DPoint* GPoint,TDegreeRule* Rule)
{
	if ( GPoint->Rules.Count > 2 )
    	throw EMyException("<TFlowInfo::RegisterPointFreedomDegree>: Îøèáêà");
    //-----------------------------------------------------------------------
    ...
    //ïîâòîðíîãî íàëîæåíèÿ îãðàíè÷åíèé íå áóäåò ò.ê. â UpdateSkeletonAndSequenceIfNeed
    //ïðîâåðÿåòñÿ
    for (int i=0;i<Skeletons.Count;i++)
...
bool TFlowInfo::Ensure_Not_Skeleton_Links(TG2DPoint* GPoint)
{
    ...
    while (IsFindedLink)
    {
        ...
        for ( int i=0;i<UpdatePoints.Count;i++ )
        {
            ...
            {
                ErrorMsg("<TFlowInfo::Ensure_Not_Skeleton_Links>: Ýñêèç ïåðåîïðåäåëåí.");
                return false;
            }
...
//    ThereIsAFullLinks(FlowInfo);
    ...
    while ( Result && FlowInfo.NotSkeletonLinks.Count )
    {
        ...
        //âñå òî÷êè ïëîõèå
        //åñëè áåðåì îäíó èç ïëîõèõ òî÷åê âíóòðè ñêåëåòîíà,
        //òî ëó÷øå óæå ñ rules
...
bool TFlowInfo::CreateInfluence(TG2DPoint* Point)
{
    bool Result = true;
    //---------------------------------
    FindSkeletons();//ñíà÷àëà ñêåëåòîíû à ïîòîì ñåðâèñ
    //-------Apply-Fixed---------------
    //---------------------------------------------------
    //Find Fixed Cuts
